...
#define NOTIFICATION_AREA_ICON "gde2-panel-notification-area"
...
static void (*parent_class_change_background)(Gde2PanelApplet* panel_applet, Gde2PanelAppletBackgroundType type, GdkRGBA* color, cairo_pattern_t* pattern);
#else
static void (*parent_class_change_background)(Gde2PanelApplet* panel_applet, Gde2PanelAppletBackgroundType type, GdkColor* color, GdkPixmap* pixmap);
#endif
static void (*parent_class_change_orient)(Gde2PanelApplet       *panel_applet, Gde2PanelAppletOrient  orient);



static GtkOrientation
get_gtk_orientation_from_applet_orient (Gde2PanelAppletOrient orient)
{
  switch (orient)
    {
...
static void help_cb(GtkAction* action, NaTrayApplet* applet)
{
	GError* error = NULL;
	char* uri;
	#define NA_HELP_DOC "gde2-user-guide"
...
na_tray_applet_realize (GtkWidget *widget)
{
  ...
  orient = gde2_panel_applet_get_orient (GDE2_PANEL_APPLET (widget));
  ...
  gde2_panel_applet_setup_menu_from_file(GDE2_PANEL_APPLET(applet), ui_path, action_group);
  g_free(ui_path);
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void
na_tray_applet_change_background(Gde2PanelApplet* panel_applet, Gde2PanelAppletBackgroundType type, GdkRGBA* color, cairo_pattern_t* pattern)
#else
static void
na_tray_applet_change_background(Gde2PanelApplet* panel_applet, Gde2PanelAppletBackgroundType type, GdkColor* color, GdkPixmap* pixmap)
#endif
{
...
static void
na_tray_applet_change_orient (Gde2PanelApplet       *panel_applet,
                              Gde2PanelAppletOrient  orient)
{
  NaTrayApplet *applet = NA_TRAY_APPLET (panel_applet);
...
na_tray_applet_class_init (NaTrayAppletClass *class)
{
  ...
  Gde2PanelAppletClass *applet_class = GDE2_PANEL_APPLET_CLASS (class);
...
na_tray_applet_init (NaTrayApplet *applet)
{
  ...
  gde2_panel_applet_set_flags (GDE2_PANEL_APPLET (applet),
                          GDE2_PANEL_APPLET_HAS_HANDLE|GDE2_PANEL_APPLET_EXPAND_MINOR);

#if !GTK_CHECK_VERSION (3, 0, 0)
  gde2_panel_applet_set_background_widget (GDE2_PANEL_APPLET (applet),
                                      GTK_WIDGET (applet));
#endif
#if GTK_CHECK_VERSION (3, 19, 0)
/* deprecated with gtk+-3.19.0 */
...
static gboolean
applet_factory (Gde2PanelApplet *applet,
                const gchar *iid,
                gpointer     user_data)
{
  ...
  return TRUE;
	...
	GDE2_PANEL_APPLET_IN_PROCESS_FACTORY ("NotificationAreaAppletFactory",
				 NA_TYPE_TRAY_APPLET,
				 "NotificationArea",
				 applet_factory,
				 NULL)
#else
	GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY ("NotificationAreaAppletFactory",
				  NA_TYPE_TRAY_APPLET,
				  "NotificationArea",
				  applet_factory,
				  NULL)
