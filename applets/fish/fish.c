...
#define FISH_ICON "gde2-panel-fish"

#define FISH_SCHEMA      "org.gde2.panel.applet.fish"
...
#define LOCKDOWN_SCHEMA                       "org.gde2.lockdown"
...
static void show_help(FishApplet* fish, const char* link_id)
{
	GError *error = NULL;
	char   *uri;
#define FISH_HELP_DOC "gde2-fish"
...
static void rotate_changed_notify(GSettings* settings, gchar* key, FishApplet* fish)
{
	...
	if (fish->orientation == GDE2_PANEL_APPLET_ORIENT_LEFT ||
	    fish->orientation == GDE2_PANEL_APPLET_ORIENT_RIGHT)
		update_pixmap (fish);
...
{
	Gde2PanelApplet *applet = (Gde2PanelApplet *) fish;

	fish->settings = gde2_panel_applet_settings_new (applet, FISH_SCHEMA);
...
static void update_pixmap(FishApplet* fish)
{
	...
	if (fish->rotate &&
	    (fish->orientation == GDE2_PANEL_APPLET_ORIENT_LEFT ||
	     fish->orientation == GDE2_PANEL_APPLET_ORIENT_RIGHT))
		rotate = TRUE;
	...
	if (fish->orientation == GDE2_PANEL_APPLET_ORIENT_UP ||
	    fish->orientation == GDE2_PANEL_APPLET_ORIENT_DOWN) {
	...
	if (rotate) {
		if (fish->orientation == GDE2_PANEL_APPLET_ORIENT_RIGHT) {
...
#endif
{
	...
	if (fish->rotate) {
		...
		else if (fish->orientation == GDE2_PANEL_APPLET_ORIENT_LEFT)
...
static void fish_applet_change_orient(Gde2PanelApplet* applet, Gde2PanelAppletOrient orientation)
{
...
static gboolean fish_applet_fill(FishApplet* fish)
{
	...
	fish->orientation = gde2_panel_applet_get_orient (applet);
	...
	if (gde2_panel_applet_get_locked_down (applet)) {
		GtkAction *action;
...
static gboolean fishy_factory(Gde2PanelApplet* applet, const char* iid, gpointer data)
{
...
static void fish_applet_instance_init(FishApplet* fish, FishAppletClass* klass)
{
	...
	fish->orientation = GDE2_PANEL_APPLET_ORIENT_UP;
	...
	gde2_panel_applet_set_flags (GDE2_PANEL_APPLET (fish), GDE2_PANEL_APPLET_EXPAND_MINOR);

	gde2_panel_applet_set_background_widget(GDE2_PANEL_APPLET(fish), GTK_WIDGET(fish));
}

static void fish_applet_class_init(FishAppletClass* klass)
{
	Gde2PanelAppletClass* applet_class = (Gde2PanelAppletClass*) klass;
...
static GType fish_applet_get_type(void)
{
	...
	GDE2_PANEL_APPLET_IN_PROCESS_FACTORY("FishAppletFactory", fish_applet_get_type(), "That-stupid-fish", fishy_factory, NULL)
#else
	GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY("FishAppletFactory", fish_applet_get_type(), "That-stupid-fish", fishy_factory, NULL)
#endif
