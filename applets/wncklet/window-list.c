...
#define WINDOW_LIST_ICON "gde2-panel-window-list"
#define WINDOW_LIST_SCHEMA "org.gde2.panel.applet.window-list"
...
static void response_cb(GtkWidget* widget, int id, TasklistData* tasklist)
{
	...
	{
		wncklet_display_help(widget, "gde2-user-guide", "windowlist-prefs", WINDOW_LIST_ICON);
	}
	else
	{
...
static void applet_realized(Gde2PanelApplet* applet, TasklistData* tasklist)
{
	tasklist->icon_theme = gtk_icon_theme_get_for_screen(gtk_widget_get_screen(tasklist->applet));
}

static void applet_change_orient(Gde2PanelApplet* applet, Gde2PanelAppletOrient orient, TasklistData* tasklist)
{
...
static void applet_change_background(Gde2PanelApplet* applet, Gde2PanelAppletBackgroundType type, GdkColor* color, cairo_pattern_t* pattern, TasklistData* tasklist)
#else
static void applet_change_background(Gde2PanelApplet* applet, Gde2PanelAppletBackgroundType type, GdkColor* color, GdkPixmap* pixmap, TasklistData* tasklist)
#endif
{
	switch (type)
	{
...
static void applet_change_pixel_size(Gde2PanelApplet* applet, gint size, TasklistData* tasklist)
{
...
{
	tasklist->settings = gde2_panel_applet_settings_new (GDE2_PANEL_APPLET (tasklist->applet), WINDOW_LIST_SCHEMA);
...
#endif
{
	...
	gde2_panel_applet_set_size_hints(GDE2_PANEL_APPLET(tasklist->applet), size_hints, len, 0);
}
...
gboolean window_list_applet_fill(Gde2PanelApplet* applet)
{
	TasklistData* tasklist;
	...
	gde2_panel_applet_set_flags(GDE2_PANEL_APPLET(tasklist->applet), GDE2_PANEL_APPLET_EXPAND_MAJOR | GDE2_PANEL_APPLET_EXPAND_MINOR | GDE2_PANEL_APPLET_HAS_HANDLE);
	...
	tasklist->size = gde2_panel_applet_get_size(applet);
	...
	gde2_panel_applet_set_background_widget(GDE2_PANEL_APPLET(tasklist->applet), GTK_WIDGET(tasklist->applet));
	 ...
	 * example, gde2-system-monitor, o gnome-system-monitor */
	char* programpath;
	int i;
	...
	gde2_panel_applet_setup_menu_from_file(GDE2_PANEL_APPLET(tasklist->applet), ui_path, action_group);
	g_free(ui_path);

	if (gde2_panel_applet_get_locked_down(GDE2_PANEL_APPLET(tasklist->applet)))
	{
		GtkAction* action;
...
static void call_system_monitor(GtkAction* action, TasklistData* tasklist)
{
	...
	for (i = 0; i < G_N_ELEMENTS(system_monitors); i += 1)
	{
		...
		if (programpath != NULL)
		{
			...
			gde2_gdk_spawn_command_line_on_screen(gtk_widget_get_screen(tasklist->applet),
				      system_monitors[i],
				      NULL);
			return;
		}
...
static void display_help_dialog(GtkAction* action, TasklistData* tasklist)
{
	wncklet_display_help(tasklist->applet, "gde2-user-guide", "windowlist", WINDOW_LIST_ICON);
}
...
static void display_about_dialog(GtkAction* action, TasklistData* tasklist)
{
	...
	gde2_show_about_dialog(GTK_WINDOW(tasklist->applet),
