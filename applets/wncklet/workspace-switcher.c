...
#define WORKSPACE_SWITCHER_SCHEMA "org.gde2.panel.applet.workspace-switcher"

#define NEVER_SENSITIVE "never_sensitive"
#define MARCO_GENERAL_SCHEMA "org.gde2.Marco.general"
#define NUM_WORKSPACES "num-workspaces"
#define MARCO_WORKSPACES_SCHEMA "org.gde2.Marco.workspace-names"
#define WORKSPACE_NAME "name-1"

#define WORKSPACE_SWITCHER_ICON "gde2-panel-workspace-switcher"
...
static void applet_realized(Gde2PanelApplet* applet, PagerData* pager)
{
...
static void applet_unrealized(Gde2PanelApplet* applet, PagerData* pager)
{
	pager->screen = NULL;
	pager->wm = PAGER_WM_UNKNOWN;
}

static void applet_change_orient(Gde2PanelApplet* applet, Gde2PanelAppletOrient orient, PagerData* pager)
{
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void applet_change_background(Gde2PanelApplet* applet, Gde2PanelAppletBackgroundType type, GdkColor* color, cairo_pattern_t *pattern, PagerData* pager)
{
...
#else
static void applet_change_background(Gde2PanelApplet* applet, Gde2PanelAppletBackgroundType type, GdkColor* color, GdkPixmap* pixmap, PagerData* pager)
{
...
#if !GTK_CHECK_VERSION (3, 0, 0)
static gboolean applet_scroll(Gde2PanelApplet* applet, GdkEventScroll* event, PagerData* pager)
{
...
{
	pager->settings = gde2_panel_applet_settings_new (GDE2_PANEL_APPLET (pager->applet), WORKSPACE_SWITCHER_SCHEMA);
...
gboolean workspace_switcher_applet_fill(Gde2PanelApplet* applet)
{
	PagerData* pager;
	...
	gde2_panel_applet_set_flags(GDE2_PANEL_APPLET(pager->applet), GDE2_PANEL_APPLET_EXPAND_MINOR);
	...
	switch (gde2_panel_applet_get_orient(applet))
	{
		case GDE2_PANEL_APPLET_ORIENT_LEFT:
		case GDE2_PANEL_APPLET_ORIENT_RIGHT:
	...
	gde2_panel_applet_set_background_widget(GDE2_PANEL_APPLET(pager->applet), GTK_WIDGET(pager->applet));
	...
	gde2_panel_applet_setup_menu_from_file(GDE2_PANEL_APPLET(pager->applet), ui_path, action_group);
	g_free(ui_path);

	if (gde2_panel_applet_get_locked_down(GDE2_PANEL_APPLET(pager->applet)))
	{
		GtkAction *action;
...
static void display_help_dialog(GtkAction* action, PagerData* pager)
{
	wncklet_display_help(pager->applet, "gde2-user-guide", "overview-workspaces", WORKSPACE_SWITCHER_ICON);
}
...
static void display_about_dialog(GtkAction* action, PagerData* pager)
{
	...
	gde2_show_about_dialog(GTK_WINDOW(pager->applet),
...
static void response_cb(GtkWidget* widget, int id, PagerData* pager)
{
	if (id == GTK_RESPONSE_HELP)
		wncklet_display_help(widget, "gde2-user-guide", "overview-workspaces", WORKSPACE_SWITCHER_ICON);
	else
		gtk_widget_destroy(widget);
}
...
static void setup_dialog(GtkBuilder* builder, PagerData* pager)
{
	...
	if (gde2_gsettings_schema_exists(MARCO_GENERAL_SCHEMA))
		marco_general_settings = g_settings_new (MARCO_GENERAL_SCHEMA);
	if (gde2_gsettings_schema_exists(MARCO_WORKSPACES_SCHEMA))
