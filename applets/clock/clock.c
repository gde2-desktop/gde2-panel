...
#include <libgde2weather/gde2weather-xml.h>
...
struct _ClockData {
	...
	Gde2WeatherLocationEntry *location_entry;
        Gde2WeatherTimezoneMenu *zone_combo;
...
static void applet_change_orient (Gde2PanelApplet       *applet,
				  Gde2PanelAppletOrient  orient,
				  ClockData         *cd);
...
create_calendar (ClockData *cd)
{
	...
	prefs_path = gde2_panel_applet_get_preferences_path (GDE2_PANEL_APPLET (cd->applet));
	window = calendar_window_new (&cd->current_time,
				      prefs_path,
				      cd->orient == GDE2_PANEL_APPLET_ORIENT_UP);
	g_free (prefs_path);
	...
	g_signal_connect (window, "key_press_event",
        ...
        gtk_widget_set_name(window, "Gde2PanelPopupWindow");
...
position_calendar_popup (ClockData *cd)
{
	 ...
	 */
	switch (cd->orient) {
	case GDE2_PANEL_APPLET_ORIENT_RIGHT:
		x += button_w;
		...
		break;
	case GDE2_PANEL_APPLET_ORIENT_LEFT:
		x -= w;
		if ((y + h) > monitor.y + monitor.height)
			y -= (y + h) - (monitor.y + monitor.height);
		...
		break;
	case GDE2_PANEL_APPLET_ORIENT_DOWN:
		y += button_h;
		...
		break;
	case GDE2_PANEL_APPLET_ORIENT_UP:
		y -= h;
		if ((x + w) > monitor.x + monitor.width)
			x -= (x + w) - (monitor.x + monitor.width);
...
create_clock_widget (ClockData *cd)
{
	...
	cd->props = NULL;
	cd->orient = -1;
	cd->size = gde2_panel_applet_get_size (GDE2_PANEL_APPLET (cd->applet));

	update_panel_weather (cd);

	/* Refresh the clock so that it paints its first state */
	refresh_clock_timeout (cd);
	applet_change_orient (GDE2_PANEL_APPLET (cd->applet),
			      gde2_panel_applet_get_orient (GDE2_PANEL_APPLET (cd->applet)),
			      cd);
}
...
update_orient (ClockData *cd)
{
	...
	if (cd->orient == GDE2_PANEL_APPLET_ORIENT_LEFT &&
	    min_width > allocation.width)
		new_angle = 270;
	else if (cd->orient == GDE2_PANEL_APPLET_ORIENT_RIGHT &&
		 min_width > allocation.width)
		new_angle = 90;
	else
		new_angle = 0;
...
static void
applet_change_orient (Gde2PanelApplet       *applet,
		      Gde2PanelAppletOrient  orient,
		      ClockData         *cd)
{
        GtkOrientation o;
                                ...
                                ClockData	*cd)
{
	...
	if (cd->orient == GDE2_PANEL_APPLET_ORIENT_LEFT ||
	    cd->orient == GDE2_PANEL_APPLET_ORIENT_RIGHT)
                ...
                ClockData    *cd)
{
	...
	data.cd = cd;
        ...
        cur = gde2_panel_applet_settings_get_gslist (settings, key);
...
{
        cd->settings = gde2_panel_applet_settings_new (GDE2_PANEL_APPLET (cd->applet), CLOCK_SCHEMA);
...
static gboolean
fill_clock_applet (Gde2PanelApplet *applet)
{
	ClockData      *cd;
	...
	gde2_panel_applet_set_background_widget (GDE2_PANEL_APPLET (cd->applet),
	...
	gde2_panel_applet_setup_menu_from_file (GDE2_PANEL_APPLET (cd->applet),
					   ui_path, action_group);
        g_free (ui_path);

	if (gde2_panel_applet_get_locked_down (GDE2_PANEL_APPLET (cd->applet))) {
...
save_cities_store (ClockData *cd)
{
        ...
        gde2_panel_applet_settings_set_glist (cd->settings, KEY_CITIES, locs);
...
run_prefs_edit_save (GtkButton *button, ClockData *cd)
{
        ...
        Gde2WeatherLocation *gloc;
        gfloat lat = 0;
        gfloat lon = 0;

        timezone = gde2weather_timezone_menu_get_tzid (cd->zone_combo);
        if (!timezone) {
                edit_hide (NULL, cd);
                return;
        }
        ...
        gloc = gde2weather_location_entry_get_location (cd->location_entry);
        if (gloc) {
                city = gde2weather_location_get_city_name (gloc);
                weather_code = gde2weather_location_get_code (gloc);
        }

        if (gde2weather_location_entry_has_custom_text (cd->location_entry)) {
...
{
        if (loc != NULL) {
                gde2weather_timezone_menu_set_tzid (cd->zone_combo,
                                                 clock_location_get_timezone (loc));
        } else {
                gde2weather_timezone_menu_set_tzid (cd->zone_combo, NULL);
        }
}
...
location_changed (GObject *object, GParamSpec *param, ClockData *cd)
{
        ...
        gloc = gde2weather_location_entry_get_location (entry);

	latlon_valid = gloc && gde2weather_location_has_coords (gloc);
        if (latlon_valid)
                gde2weather_location_get_coords (gloc, &latitude, &longitude);
        update_coords (cd, latlon_valid, latitude, longitude);

        zone = gloc ? gde2weather_location_get_timezone (gloc) : NULL;
        if (zone)
                gde2weather_timezone_menu_set_tzid (cd->zone_combo, gde2weather_timezone_get_tzid (zone));
        else
                gde2weather_timezone_menu_set_tzid (cd->zone_combo, NULL);

        if (gloc)
                gde2weather_location_unref (gloc);
}
...
edit_clear (ClockData *cd)
{
        ...
        gde2weather_location_entry_set_location (cd->location_entry, NULL);
        gde2weather_timezone_menu_set_tzid (cd->zone_combo, NULL);
...
edit_tree_row (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
        ...
        gde2weather_location_entry_set_city (cd->location_entry,
...
fill_prefs_window (ClockData *cd)
{
	...
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (widget), renderer, "text", 0, NULL);
		...
		gtk_list_store_insert_with_values (store, &iter, -1,
						   0, gde2weather_prefs_get_temp_display_name (temperatures[i]),
						   -1);
	...
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (widget), renderer, "text", 0, NULL);
		...
		gtk_list_store_insert_with_values (store, &iter, -1,
						   0, gde2weather_prefs_get_speed_display_name (speeds[i]),
						   -1);
...
ensure_prefs_window_is_created (ClockData *cd)
{
	...
	if (!clock_locale_supports_am_pm ())
        ...
        world = gde2weather_location_new_world (FALSE);

        location_box = _clock_get_widget (cd, "edit-location-name-box");
        cd->location_entry = GDE2WEATHER_LOCATION_ENTRY (gde2weather_location_entry_new (world));
        ...
        cd->zone_combo = GDE2WEATHER_TIMEZONE_MENU (gde2weather_timezone_menu_new (world));
...
display_help_dialog (GtkAction *action,
                     ClockData *cd)
{
	clock_utils_display_help (cd->applet, "gde2-clock", NULL);
}
...
static void display_about_dialog(GtkAction* action, ClockData* cd)
{
	...
	gde2_show_about_dialog(NULL,
		...
		"version", VERSION,
		"website", "http://gde2-desktop.org/",
		NULL);
}

static gboolean
clock_factory (Gde2PanelApplet *applet,
	       const char  *iid,
	       gpointer     data)
{
...
GDE2_PANEL_APPLET_IN_PROCESS_FACTORY ("ClockAppletFactory",
                                 PANEL_TYPE_APPLET,
                                 "ClockApplet",
                                 clock_factory,
                                 NULL)
#else
GDE2_PANEL_APPLET_OUT_PROCESS_FACTORY ("ClockAppletFactory",
                                  PANEL_TYPE_APPLET,
                                  "ClockApplet",
                                  clock_factory,
                                  NULL)
