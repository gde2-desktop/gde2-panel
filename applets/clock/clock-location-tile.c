			    ...
			    ClockFormat clock_format)
{
	...
	if (strcmp (apparent, temp) != 0 &&
	    /* FMQ: it's broken to read from another module's translations; add some API to libgde2weather. */
            strcmp (apparent, dgettext ("gde2-applets-2.0", "Unknown")) != 0)
	...
	wind = weather_info_get_wind (info);
        if (strcmp (apparent, dgettext ("gde2-applets-2.0", "Unknown")) != 0)
