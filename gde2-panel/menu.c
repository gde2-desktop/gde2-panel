...
#include <libgde2-desktop/gde2-gsettings.h>
...
static inline gboolean desktop_is_home_dir(void)
{
	...
	if (gde2_gsettings_schema_exists (CAJA_PREFS_SCHEMA)) {
...
activate_app_def (GtkWidget      *menuitem,
		  Gde2MenuTreeEntry *entry)
{
	const char       *path;

	path = gde2menu_tree_entry_get_desktop_file_path (entry);
...
panel_create_menu (void)
{
	...
	retval = gtk_menu_new ();
	gtk_widget_set_name (retval, "gde2-panel-main-menu");
		  ...
		  Gde2MenuTreeEntry *entry)
{
	...
	panel_launcher_create (toplevel,
			       position,
			       gde2menu_tree_entry_get_desktop_file_path (entry));
}
		    ...
		    Gde2MenuTreeEntry *entry)
{
	...
	source = gde2menu_tree_entry_get_desktop_file_path (entry);
...
static void add_drawers_from_dir (Gde2MenuTreeDirectory *directory,
				  int                 pos,
				  const char         *toplevel_id);

static void
add_drawers_from_alias (Gde2MenuTreeAlias *alias,
			const char     *toplevel_id)
{
	Gde2MenuTreeItem *aliased_item;

	aliased_item = gde2menu_tree_alias_get_item (alias);

	switch (gde2menu_tree_item_get_type (aliased_item)) {
	case GDE2MENU_TREE_ITEM_DIRECTORY:
		add_drawers_from_dir (GDE2MENU_TREE_DIRECTORY (aliased_item),
				      G_MAXINT/2,
				      toplevel_id);
		break;
	...
	case GDE2MENU_TREE_ITEM_ENTRY:
		panel_launcher_create_with_id (toplevel_id,
					       G_MAXINT/2,
					       gde2menu_tree_entry_get_desktop_file_path (GDE2MENU_TREE_ENTRY (aliased_item)));
		break;
	...
	gde2menu_tree_item_unref (aliased_item);
}

static void
add_drawers_from_dir (Gde2MenuTreeDirectory *directory,
		      int                 pos,
		      const char         *toplevel_id)
{
	const char *name;
	...
	name = gde2menu_tree_directory_get_name (directory);
	icon = gde2menu_tree_directory_get_icon (directory);
	...
	items = gde2menu_tree_directory_get_contents (directory);
	for (l = items; l; l = l->next) {
		Gde2MenuTreeItem *item = l->data;

		switch (gde2menu_tree_item_get_type (item)) {
		case GDE2MENU_TREE_ITEM_ENTRY:
			panel_launcher_create_with_id (attached_toplevel_id,
						       G_MAXINT/2,
						       gde2menu_tree_entry_get_desktop_file_path (GDE2MENU_TREE_ENTRY (item)));
			break;

		case GDE2MENU_TREE_ITEM_DIRECTORY:
			add_drawers_from_dir (GDE2MENU_TREE_DIRECTORY (item),
					      G_MAXINT/2,
					      attached_toplevel_id);
			break;

		case GDE2MENU_TREE_ITEM_ALIAS:
			add_drawers_from_alias (GDE2MENU_TREE_ALIAS (item), attached_toplevel_id);
			break;
...

{
	...
	directory = gde2menu_tree_item_get_parent (GDE2MENU_TREE_ITEM (entry));
		   ...
		   Gde2MenuTreeEntry *entry)
{
	...
	tree = gde2menu_tree_directory_get_tree (directory);
	if (!tree) {
		gde2menu_tree_item_unref (directory);
	...
	menu_filename = gde2menu_tree_get_menu_file (tree);
	...
	menu_path = gde2menu_tree_directory_make_path (directory, NULL);
	...
	panel_menu_button_create (panel->toplevel,
				  ...
				  menu_path,
				  TRUE,
				  gde2menu_tree_directory_get_name (directory));
...
{
	static GtkTargetEntry menu_item_targets[] = {
		{ "application/x-gde2-panel-applet-internal", 0, 0 }
	};
...
submenu_to_display (GtkWidget *menu)
{
	...
	if (!directory) {
		...
		directory = gde2menu_tree_get_directory_from_path (tree,
								menu_path);

		g_object_set_data_full (G_OBJECT (menu),
					"panel-menu-tree-directory",
					directory,
					(GDestroyNotify) gde2menu_tree_item_unref);
	}
...
static GtkWidget *
create_fake_menu (Gde2MenuTreeDirectory *directory)
{
	GtkWidget *menu;
	guint      idle_id;
	...
	g_object_set_data_full (G_OBJECT (menu),
				"panel-menu-tree-directory",
				gde2menu_tree_item_ref (directory),
				(GDestroyNotify) gde2menu_tree_item_unref);
		      ...
		      Gde2MenuTreeDirectory *directory)
{
	...
	setup_menuitem_with_icon (menuitem,
				  ...
				  NULL,
				  gde2menu_tree_directory_get_icon (directory),
				  gde2menu_tree_directory_get_name (directory));
...
create_submenu (GtkWidget          *menu,
		Gde2MenuTreeDirectory *directory,
		Gde2MenuTreeDirectory *alias_directory)
{
	GtkWidget *menuitem;
	GtkWidget *submenu;
	gboolean   force_categories_icon;
...
create_header (GtkWidget       *menu,
	       Gde2MenuTreeHeader *header)
{
	Gde2MenuTreeDirectory *directory;
	GtkWidget          *menuitem;

	directory = gde2menu_tree_header_get_directory (header);
	menuitem = create_submenu_entry (menu, directory);
	gde2menu_tree_item_unref (directory);

	g_object_set_data_full (G_OBJECT (menuitem),
				"panel-gde2menu-tree.header",
				gde2menu_tree_item_ref (header),
				(GDestroyNotify) gde2menu_tree_item_unref);
		 ...
		 Gde2MenuTreeDirectory *alias_directory)
{
	...
	if (alias_directory)
		...
		g_object_set_data_full (G_OBJECT (menuitem),
					...
					gde2menu_tree_item_ref (alias_directory),
					(GDestroyNotify) gde2menu_tree_item_unref);

	setup_menuitem_with_icon (menuitem,
				  panel_menu_icon_get_size (),
				  NULL,
				  alias_directory ? gde2menu_tree_directory_get_icon (alias_directory) :
						    gde2menu_tree_entry_get_icon (entry),
				  alias_directory ? gde2menu_tree_directory_get_name (alias_directory) :
						    gde2menu_tree_entry_get_display_name (entry));

	if (alias_directory &&
	    gde2menu_tree_directory_get_comment (alias_directory))
		panel_util_set_tooltip_text (menuitem,
					     gde2menu_tree_directory_get_comment (alias_directory));
	else if	(!alias_directory &&
		 gde2menu_tree_entry_get_comment (entry))
		panel_util_set_tooltip_text (menuitem,
					     gde2menu_tree_entry_get_comment (entry));
	else if	(!alias_directory &&
		 gde2menu_tree_entry_get_generic_name (entry))
		panel_util_set_tooltip_text (menuitem,
					     gde2menu_tree_entry_get_generic_name (entry));
	...
	if (!panel_lockdown_get_locked_down ()) {
		...
		if (gde2menu_tree_entry_get_icon (entry) != NULL) {
			const char *icon;
			char       *icon_no_ext;

			icon = gde2menu_tree_entry_get_icon (entry);
			    ...
			    Gde2MenuTreeAlias *alias)
{
	...
	case GDE2MENU_TREE_ITEM_DIRECTORY:
		create_submenu (menu,
				GDE2MENU_TREE_DIRECTORY (aliased_item),
				gde2menu_tree_alias_get_directory (alias));
		break;

	case GDE2MENU_TREE_ITEM_ENTRY:
		create_menuitem (menu,
				 GDE2MENU_TREE_ENTRY (aliased_item),
				 gde2menu_tree_alias_get_directory (alias));
		break;
...
static void
handle_gde2menu_tree_changed (Gde2MenuTree *tree,
			   GtkWidget *menu)
{
	guint idle_id;
...
static void
remove_gde2menu_tree_monitor (GtkWidget *menu,
			  Gde2MenuTree  *tree)
{
	gde2menu_tree_remove_monitor (tree,
				  (Gde2MenuTreeChangedFunc) handle_gde2menu_tree_changed,
				  menu);
}
			  ...
			  gboolean    always_show_image)
{
	...
	tree = gde2menu_tree_lookup (menu_file, GDE2MENU_TREE_FLAGS_NONE);
	gde2menu_tree_set_sort_key (tree, GDE2MENU_TREE_SORT_DISPLAY_NAME);

	g_object_set_data_full (G_OBJECT (menu),
				"panel-menu-tree",
				gde2menu_tree_ref (tree),
				(GDestroyNotify) gde2menu_tree_unref);
	...
	gde2menu_tree_add_monitor (tree,
			       (Gde2MenuTreeChangedFunc) handle_gde2menu_tree_changed,
			       menu);
	g_signal_connect (menu, "destroy",
			  G_CALLBACK (remove_gde2menu_tree_monitor), tree);
			      ...
			      Gde2MenuTreeDirectory *directory)
{
	...
	for (l = items; l; l = l->next) {
		...
		if (add_separator ||
		    gde2menu_tree_item_get_type (item) == GDE2MENU_TREE_ITEM_SEPARATOR) {
		...
		switch (gde2menu_tree_item_get_type (item)) {
		case GDE2MENU_TREE_ITEM_DIRECTORY:
			create_submenu (menu, GDE2MENU_TREE_DIRECTORY (item), NULL);
			break;

		case GDE2MENU_TREE_ITEM_ENTRY:
			create_menuitem (menu, GDE2MENU_TREE_ENTRY (item), NULL);
			break;
		...
		case GDE2MENU_TREE_ITEM_ALIAS:
			create_menuitem_from_alias (menu, GDE2MENU_TREE_ALIAS (item));
			break;

		case GDE2MENU_TREE_ITEM_HEADER:
			create_header (menu, GDE2MENU_TREE_HEADER (item));
			break;
...
GtkWidget* create_main_menu(PanelWidget* panel)
{
	...
	main_menu = create_applications_menu("gde2-applications.menu", NULL, TRUE);
