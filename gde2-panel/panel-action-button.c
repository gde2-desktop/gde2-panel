...
/* taken from deprecated gde2-conf code */
static const gchar*
...
{
	gde2_panel_applet_add_callback (button->priv->info,
				   "activate",
				   NULL,
...
panel_action_logout (GtkWidget *widget)
{
	...
	if (gde2_gsettings_schema_exists (GDE2_SESSION_SCHEMA)) {
		GSettings *msm_settings;
		msm_settings = g_settings_new (GDE2_SESSION_SCHEMA);
		prompt = g_settings_get_boolean (msm_settings, GDE2_SESSION_LOGOUT_PROMPT_KEY);
...
panel_action_search (GtkWidget *widget)
{
	...
	panel_launch_desktop_file_with_fallback ("gde2-search-tool.desktop",
						 "gde2-search-tool",
						 screen, NULL);
}
...
panel_action_connect_server (GtkWidget *widget)
{
	...
	gde2_gdk_spawn_command_line_on_screen (screen, command, &error);
	g_free (command);
			  ...
			  const char            *id)
{
	...
	button->priv->info = gde2_panel_applet_register (GTK_WIDGET (button),
						    NULL, NULL,
				 ...
				 const char        *callback_name)
{
	...
	if (!strcmp (callback_name, "help")) {
		...
		panel_show_help (screen, "gde2-user-guide",
