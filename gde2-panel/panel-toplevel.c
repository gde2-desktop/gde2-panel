					  ...
					  gboolean       for_end_position)
{
	...
	/* For the initial animation, we anigde2 from outside the screen, and
		     ...
		     GtkDirectionType  direction)
{
	...
	if (toplevel->priv->anigde2 && gtk_widget_get_realized (GTK_WIDGET (toplevel)))
			     ...
			     GParamSpec   *pspec)
{
	...
	case PROP_AUTOHIDE_SIZE:
		...
		break;
	case PROP_ANIGDE2:
		panel_toplevel_set_anigde2 (toplevel, g_value_get_boolean (value));
		break;
			     ...
			     GParamSpec *pspec)
{
	...
	case PROP_AUTOHIDE_SIZE:
		...
		break;
	case PROP_ANIGDE2:
		g_value_set_boolean (value, toplevel->priv->anigde2);
		break;
	case PROP_ANIMATION_SPEED:
...
panel_toplevel_class_init (PanelToplevelClass *klass)
{
	...
	g_object_class_install_property (
		...
		g_param_spec_enum (
			"animation-speed",
			"Animation Speed",
			"The speed at which to anigde2 panel hiding/showing",
...
void
panel_toplevel_set_anigde2 (PanelToplevel *toplevel,
			    gboolean       anigde2)
{
	g_return_if_fail (PANEL_IS_TOPLEVEL (toplevel));

	anigde2 = anigde2 != FALSE;

	if (toplevel->priv->anigde2 == anigde2)
		return;

	toplevel->priv->anigde2 = anigde2;

	g_object_notify (G_OBJECT (toplevel), "anigde2");
}

gboolean
panel_toplevel_get_anigde2 (PanelToplevel *toplevel)
{
	g_return_val_if_fail (PANEL_IS_TOPLEVEL (toplevel), FALSE);

	return toplevel->priv->anigde2;
}
