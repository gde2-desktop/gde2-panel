...
struct _Gde2PanelAppletContainerPrivate {
...
#define GDE2_PANEL_APPLET_CONTAINER_GET_PRIVATE(o) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((o), PANEL_TYPE_APPLET_CONTAINER, Gde2PanelAppletContainerPrivate))

#define GDE2_PANEL_APPLET_BUS_NAME            "org.gde2.panel.applet.%s"
#define GDE2_PANEL_APPLET_FACTORY_INTERFACE   "org.gde2.panel.applet.AppletFactory"
...
static gboolean gde2_panel_applet_container_plug_removed (Gde2PanelAppletContainer *container);

G_DEFINE_TYPE (Gde2PanelAppletContainer, gde2_panel_applet_container, GTK_TYPE_EVENT_BOX);

GQuark gde2_panel_applet_container_error_quark (void)
{
	return g_quark_from_static_string ("gde2-panel-applet-container-error-quark");
}

static void gde2_panel_applet_container_init(Gde2PanelAppletContainer* container)
{
	container->priv = GDE2_PANEL_APPLET_CONTAINER_GET_PRIVATE (container);

	container->priv->socket = gtk_socket_new ();
	g_signal_connect_swapped (container->priv->socket, "plug-removed", G_CALLBACK (gde2_panel_applet_container_plug_removed), container);
...
static void
gde2_panel_applet_container_cancel_pending_operations (Gde2PanelAppletContainer *container)
{
	GList *keys, *l;
...
static void
gde2_panel_applet_container_dispose (GObject *object)
{
	Gde2PanelAppletContainer *container = GDE2_PANEL_APPLET_CONTAINER (object);

	if (container->priv->pending_ops) {
		gde2_panel_applet_container_cancel_pending_operations (container);
	...
	G_OBJECT_CLASS (gde2_panel_applet_container_parent_class)->dispose (object);
}

static void
gde2_panel_applet_container_class_init (Gde2PanelAppletContainerClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (Gde2PanelAppletContainerPrivate));

	gobject_class->dispose = gde2_panel_applet_container_dispose;

	signals[APPLET_BROKEN] =
		g_signal_new ("applet-broken",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (Gde2PanelAppletContainerClass, applet_broken),
			      NULL,
			      NULL,
	...
	signals[APPLET_MOVE] =
		g_signal_new ("applet-move",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (Gde2PanelAppletContainerClass, applet_move),
			      NULL,
			      NULL,
			      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE,
	...
	signals[APPLET_REMOVE] =
		g_signal_new ("applet-remove",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (Gde2PanelAppletContainerClass, applet_remove),
			      NULL,
			      NULL,
			      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE,
	...
	signals[APPLET_LOCK] =
		g_signal_new ("applet-lock",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (Gde2PanelAppletContainerClass, applet_lock),
			      NULL,
			      NULL,
	...
	signals[CHILD_PROPERTY_CHANGED] =
		g_signal_new ("child-property-changed",
			      ...
			      G_STRUCT_OFFSET (Gde2PanelAppletContainerClass, child_property_changed),
			      NULL,
			      NULL,
...
gde2_panel_applet_container_child_property_get_info (const gchar *property_name)
{
	gint i;
...
GtkWidget *
gde2_panel_applet_container_new (void)
{
	GtkWidget *container;
...
static gboolean
gde2_panel_applet_container_plug_removed (Gde2PanelAppletContainer *container)
{
	if (!container->priv->applet_proxy)
		return FALSE;
...
static void
gde2_panel_applet_container_child_signal (GDBusProxy           *proxy,
				     gchar                *sender_name,
				     gchar                *signal_name,
				     GVariant             *parameters,
		   ...
		   gpointer      user_data)
{
	...
	container = GDE2_PANEL_APPLET_CONTAINER (g_async_result_get_source_object (G_ASYNC_RESULT (result)));

	container->priv->applet_proxy = proxy;
	g_signal_connect (container->priv->applet_proxy, "g-signal",
			  G_CALLBACK (gde2_panel_applet_container_child_signal),
			  container);
		     ...
		     AppletFactoryData *data)
{
	...
	container = GDE2_PANEL_APPLET_CONTAINER (g_async_result_get_source_object (G_ASYNC_RESULT (data->result)));
	container->priv->bus_name = g_strdup (name_owner);
	object_path = g_strdup_printf (GDE2_PANEL_APPLET_FACTORY_OBJECT_PATH, data->factory_id);
	g_dbus_connection_call (connection,
				name_owner,
				object_path,
				GDE2_PANEL_APPLET_FACTORY_INTERFACE,
				"GetApplet",
				data->parameters,
				G_VARIANT_TYPE ("(ou)"),
...
static void
gde2_panel_applet_container_get_applet (Gde2PanelAppletContainer *container,
				   GdkScreen            *screen,
				   const gchar          *iid,
				   GVariant             *props,
				   GCancellable         *cancellable,
				   ...
				   gpointer              user_data)
{
	...
	result = g_simple_async_result_new (G_OBJECT (container),
					    callback,
					    user_data,
					    gde2_panel_applet_container_get_applet);

	applet_id = g_strrstr (iid, "::");
	if (!applet_id) {
		g_simple_async_result_set_error (result,
						 GDE2_PANEL_APPLET_CONTAINER_ERROR,
	...
	bus_name = g_strdup_printf (GDE2_PANEL_APPLET_BUS_NAME, factory_id);
...
void
gde2_panel_applet_container_add (Gde2PanelAppletContainer *container,
			    GdkScreen            *screen,
			    const gchar          *iid,
			    GCancellable         *cancellable,
			    GAsyncReadyCallback   callback,
			    ...
			    GVariant             *properties)
{
	...
	gde2_panel_applet_container_get_applet (container, screen, iid, properties,
					   cancellable, callback, user_data);
}

gboolean
gde2_panel_applet_container_add_finish (Gde2PanelAppletContainer *container,
				   GAsyncResult         *result,
				   GError              **error)
{
	GSimpleAsyncResult *simple = G_SIMPLE_ASYNC_RESULT (result);

	g_warn_if_fail (g_simple_async_result_get_source_tag (simple) == gde2_panel_applet_container_get_applet);
...
void
gde2_panel_applet_container_child_set (Gde2PanelAppletContainer *container,
				  ...
				  gpointer              user_data)
{
	...
	info = gde2_panel_applet_container_child_property_get_info (property_name);
	if (!info) {
	...
	result = g_simple_async_result_new (G_OBJECT (container),
					    callback,
					    user_data,
					    gde2_panel_applet_container_child_set);
...
gboolean
gde2_panel_applet_container_child_set_finish (Gde2PanelAppletContainer *container,
					 GAsyncResult         *result,
					 GError              **error)
{
	GSimpleAsyncResult *simple = G_SIMPLE_ASYNC_RESULT (result);

	g_warn_if_fail (g_simple_async_result_get_source_tag (simple) == gde2_panel_applet_container_child_set);
...
void
gde2_panel_applet_container_child_get (Gde2PanelAppletContainer *container,
				  const gchar          *property_name,
				  GCancellable         *cancellable,
				  GAsyncReadyCallback   callback,
				  gpointer              user_data)
{
	...
	result = g_simple_async_result_new (G_OBJECT (container),
					    callback,
					    user_data,
					    gde2_panel_applet_container_child_get);
	if (cancellable)
		g_object_ref (cancellable);
	else
		cancellable = g_cancellable_new ();
...
GVariant *
gde2_panel_applet_container_child_get_finish (Gde2PanelAppletContainer *container,
					 GAsyncResult         *result,
					 GError              **error)
{
	GSimpleAsyncResult *simple = G_SIMPLE_ASYNC_RESULT (result);

	g_warn_if_fail (g_simple_async_result_get_source_tag (simple) == gde2_panel_applet_container_child_get);
...
void
gde2_panel_applet_container_child_popup_menu (Gde2PanelAppletContainer *container,
					 guint                 button,
					 guint32               timestamp,
					 GCancellable         *cancellable,
					 GAsyncReadyCallback   callback,
					 gpointer              user_data)
{
	...
	result = g_simple_async_result_new (G_OBJECT (container),
					    callback,
					    user_data,
					    gde2_panel_applet_container_child_popup_menu);
...
gboolean
gde2_panel_applet_container_child_popup_menu_finish (Gde2PanelAppletContainer *container,
						GAsyncResult         *result,
						GError              **error)
{
	GSimpleAsyncResult *simple = G_SIMPLE_ASYNC_RESULT (result);

	g_warn_if_fail (g_simple_async_result_get_source_tag (simple) == gde2_panel_applet_container_child_popup_menu);
