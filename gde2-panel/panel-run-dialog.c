...
#define PANEL_RUN_SCHEMA "org.gde2.panel"
#define PANEL_RUN_HISTORY_KEY "history-gde2-run"
			   ...
			   GtkWidget      *run_dialog)
{
	...
	case GTK_RESPONSE_HELP:
		panel_show_help (gtk_window_get_screen (GTK_WINDOW (run_dialog)),
				 "gde2-user-guide", "gospanel-23", NULL);
		break;
	default:
		break;
	}
...
static int
compare_applications (Gde2MenuTreeEntry *a,
		      Gde2MenuTreeEntry *b)
{
	return g_utf8_collate (gde2menu_tree_entry_get_display_name (a),
			       gde2menu_tree_entry_get_display_name (b));
}

static GSList *get_all_applications_from_dir (Gde2MenuTreeDirectory *directory,
					      GSList            *list);

static GSList *
get_all_applications_from_alias (Gde2MenuTreeAlias *alias,
				 GSList         *list)
{
	...
	case GDE2MENU_TREE_ITEM_ENTRY:
		list = g_slist_append (list, gde2menu_tree_item_ref (aliased_item));
		break;

	case GDE2MENU_TREE_ITEM_DIRECTORY:
		list = get_all_applications_from_dir (GDE2MENU_TREE_DIRECTORY (aliased_item),
						      list);
		break;
...
static GSList *
get_all_applications_from_dir (Gde2MenuTreeDirectory *directory,
			       GSList             *list)
{
	GSList *items;
	GSList *l;
	...
	for (l = items; l; l = l->next) {
		...
		case GDE2MENU_TREE_ITEM_ENTRY:
			list = g_slist_append (list, gde2menu_tree_item_ref (l->data));
			break;
...
static GSList* get_all_applications(void)
{
	...
	root = gde2menu_tree_get_root_directory(tree);
...
panel_run_dialog_add_items_idle (PanelRunDialog *dialog)
{
	...
	for (l = all_applications; l; l = next) {
		Gde2MenuTreeEntry *entry = l->data;
		const char     *entry_name;

		next = l->next;

		entry_name = gde2menu_tree_entry_get_display_name (entry);
		if (prev_name && entry_name && strcmp (entry_name, prev_name) == 0) {
			gde2menu_tree_item_unref (entry);
	...
	for (l = all_applications; l; l = l->next) {
		Gde2MenuTreeEntry *entry = l->data;
		GtkTreeIter    iter;
		const gchar *icon = gde2menu_tree_entry_get_icon (entry);
		...
		gtk_list_store_set (dialog->program_list_store, &iter,
				    COLUMN_GICON,     gicon,
				    COLUMN_NAME,      gde2menu_tree_entry_get_display_name (entry),
				    COLUMN_COMMENT,   gde2menu_tree_entry_get_comment (entry),
				    COLUMN_EXEC,      gde2menu_tree_entry_get_exec (entry),
				    COLUMN_PATH,      gde2menu_tree_entry_get_desktop_file_path (entry),
				    COLUMN_VISIBLE,   TRUE,
				    -1);
