...
#include <libgde2-desktop/gde2-dconf.h>
...
panel_profile_find_new_id (PanelGSettingsKeyType type)
{
	...
	existing_ids = gde2_dconf_list_subdirs (dir, TRUE);
...
panel_profile_get_attached_object_settings (PanelToplevel *toplevel)
{
	...
	id = gde2_panel_applet_get_id_by_widget (attach_widget);
...
#define UPDATE_CENTERED(k, n, n2)                                   \
	...
	else UPDATE_BOOL ("enable-animations", anigde2)
						   ...
						   const char        *id)
{
    ...
    if (new_id != NULL) {
		gde2_gsettings_append_strv (profile_settings,
		                            key_from_type (type),
		                            new_id);
		g_free (new_id);
    }
...
panel_profile_remove_from_list (PanelGSettingsKeyType  type,
								const char        *id)
{
	gde2_gsettings_remove_all_from_strv (profile_settings,
	                                     key_from_type (type),
	                                     id);
}
...
#define GET_ENUM(k, fn)                                           \
	...
	GET_BOOL ("enable-animations", anigde2);
...
panel_profile_load_and_show_toplevel (char *toplevel_id)
{
	...
	if (!loading_queued_applets)
		gde2_panel_applet_load_queued_applets (FALSE);
...
panel_profile_delete_object (AppletInfo *applet_info)
{
	...
	id = gde2_panel_applet_get_id (applet_info);
...
panel_profile_load_object (char *id)
{
	...
	gde2_panel_applet_queue_applet_to_load (id,
					   object_type,
					   toplevel_id,
					   position,
					   right_stick,
...
panel_profile_destroy_object (const char *id)
{
	...
	info = gde2_panel_applet_get_by_id (id);
						  ...
						  const char            *id)
{
	...
	if (type == PANEL_GSETTINGS_TOPLEVELS) {
		...
		gde2_dconf_recursive_reset (subdir, NULL);
		g_free (subdir);
	}
	else if (type == PANEL_GSETTINGS_OBJECTS) {
		gchar *subdir;
	...
	if (dir != NULL) {
		gde2_dconf_recursive_reset (dir, NULL);
		g_free (dir);
	}
}
									   ...
									   gpointer   user_data)
{
	...
	toplevel_ids = gde2_gsettings_strv_to_gslist ((const gchar **) toplevel_ids_strv);
...
panel_profile_object_id_list_update (gchar **objects)
{
	...
	object_ids = gde2_gsettings_strv_to_gslist ((const gchar **) objects);
	object_ids = panel_g_slist_make_unique (object_ids,
						(GCompareFunc) g_strcmp0,
						FALSE);

	existing_applets = gde2_panel_applet_list_applets ();
	...
	panel_profile_load_added_ids (sublist,
								  object_ids,
								  (PanelProfileGetIdFunc) gde2_panel_applet_get_id,
								  (PanelProfileLoadFunc) panel_profile_load_object,
								  (PanelProfileOnLoadQueue) gde2_panel_applet_on_load_queue);
...
{
	profile_settings = g_settings_new ("org.gde2.panel");
}
...
panel_profile_load (void)
{
	...
	gde2_panel_applet_load_queued_applets (TRUE);
}
