...
static MenuPathRootItem root_items[] = {
	{APPLICATIONS_MENU, "gde2-applications", "gde2-applications.menu"},
	{SETTINGS_MENU, "gde2-settings", "gde2-settings.menu"}
};
...
panel_menu_button_associate_panel (PanelMenuButton *button)
{
	...
	gde2_panel_applet_menu_set_recurse (GTK_MENU (button->priv->menu), "menu_panel", panel_widget);
}
			...
			const char  *id)
{
	...
	info = gde2_panel_applet_register (GTK_WIDGET (button), NULL, NULL,
	...
	gde2_panel_applet_add_callback (info, "help", GTK_STOCK_HELP, _("_Help"), NULL);

        if (!panel_lockdown_get_locked_down () &&
            (panel_is_program_in_path ("mozo") ||
	    panel_is_program_in_path ("gde2menu-simple-editor")))
		gde2_panel_applet_add_callback (info, "edit", NULL,
					   _("_Edit Menus"), NULL);
...
panel_menu_button_get_icon (PanelMenuButton *button)
{
	...
	if (!retval                     &&
		...
		if (directory)
			retval = g_strdup (gde2menu_tree_directory_get_icon (directory));
	}
			       ...
			       const char   *callback_name)
{
	...
	if (!strcmp (callback_name, "help")) {
		panel_show_help (screen, "gde2-user-guide", "gospanel-37", NULL);
		...
		if (error) {
			...
			panel_launch_desktop_file_with_fallback (
						"gde2menu-simple-editor.desktop",
						"gde2menu-simple-editor",
						screen, NULL);
		}
	}
}
