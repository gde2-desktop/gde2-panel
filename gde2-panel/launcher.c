		      ...
		      const char       *id)
{
	...
	launcher->info = gde2_panel_applet_register (launcher->button, launcher,
						free_launcher,
	...
	gde2_panel_applet_add_callback (launcher->info,
				   "launch",
				   GTK_STOCK_EXECUTE,
				   _("_Launch"),
				   NULL);
			      ...
			      const char  *id)
{
	...
	if (launcher) {
		if (!g_settings_is_writable (settings, PANEL_OBJECT_LAUNCHER_LOCATION_KEY)) {
			...
			menu = gde2_panel_applet_get_callback (launcher->info->user_menu,
							  "properties");
			if (menu != NULL)
...
find_launcher (const char *path)
{
	...
	for (l = gde2_panel_applet_list_applets (); l; l = l->next) {
