...
static void gde2_panel_applet_frame_activating_free (Gde2PanelAppletFrameActivating *frame_act);

static void gde2_panel_applet_frame_loading_failed  (const char  *iid,
					        PanelWidget *panel,
					        const char  *id);

static void gde2_panel_applet_frame_load            (const gchar *iid,
						PanelWidget *panel,
						gboolean     locked,
						int          position,
						gboolean     exactpos,
...
struct _Gde2PanelAppletFrameActivating {
	gboolean     locked;
	PanelWidget *panel;
	int          position;
	gboolean     exactpos;
...
/* Gde2PanelAppletFrame implementation */

G_DEFINE_TYPE (Gde2PanelAppletFrame, gde2_panel_applet_frame, GTK_TYPE_EVENT_BOX)

#define GDE2_PANEL_APPLET_FRAME_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), PANEL_TYPE_APPLET_FRAME, Gde2PanelAppletFramePrivate))

#define HANDLE_SIZE 10
#define GDE2_PANEL_APPLET_PREFS_PATH "/org/gde2/panel/objects/%s/prefs/"

struct _Gde2PanelAppletFramePrivate {
	PanelWidget     *panel;
...
static gboolean
gde2_panel_applet_frame_draw (GtkWidget *widget,
                         cairo_t   *cr)
{
	Gde2PanelAppletFrame *frame = GDE2_PANEL_APPLET_FRAME (widget);
	GtkStyleContext *context;
	GtkStateFlags     state;
	cairo_pattern_t  *bg_pattern;
	PanelBackground  *background;

	if (GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->draw)
		GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->draw (widget, cr);
...
#else
static void
gde2_panel_applet_frame_paint (GtkWidget    *widget,
			  GdkRectangle *area)
{
	Gde2PanelAppletFrame *frame;

	frame = GDE2_PANEL_APPLET_FRAME (widget);
...
static gboolean gde2_panel_applet_frame_expose(GtkWidget* widget, GdkEventExpose* event)
{
	if (gtk_widget_is_drawable (widget))
	{
		GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->expose_event (widget, event);

		gde2_panel_applet_frame_paint (widget, &event->area);
	}
...
static void
gde2_panel_applet_frame_update_background_size (Gde2PanelAppletFrame *frame,
					   ...
					   GtkAllocation    *new_allocation)
{
	...
	gde2_panel_applet_frame_change_background (frame, background->type);
}

#if GTK_CHECK_VERSION (3, 0, 0)
static void
gde2_panel_applet_frame_get_preferred_width(GtkWidget *widget, gint *minimal_width, gint *natural_width)
{
	Gde2PanelAppletFrame *frame;
	GtkBin           *bin;
	GtkWidget        *child;
	...
	if (!frame->priv->has_handle) {
		GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->get_preferred_width (widget, minimal_width, natural_width);
		return;
	}
...
static void
gde2_panel_applet_frame_get_preferred_height(GtkWidget *widget, gint *minimal_height, gint *natural_height)
{
	Gde2PanelAppletFrame *frame;
	GtkBin           *bin;
	GtkWidget        *child;
	...
	if (!frame->priv->has_handle) {
		GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->get_preferred_height (widget, minimal_height, natural_height);
		return;
	}
...
#else
static void
gde2_panel_applet_frame_size_request (GtkWidget      *widget,
				 GtkRequisition *requisition)
{
	...
	if (!frame->priv->has_handle) {
		GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->size_request (widget, requisition);
		return;
	}
...
static void
gde2_panel_applet_frame_size_allocate (GtkWidget     *widget,
				  GtkAllocation *allocation)
{
	...
	if (!frame->priv->has_handle) {
		GTK_WIDGET_CLASS (gde2_panel_applet_frame_parent_class)->size_allocate (widget,  allocation);
		gde2_panel_applet_frame_update_background_size (frame, &old_allocation, allocation);
		return;
	}
	...
	gde2_panel_applet_frame_update_background_size (frame,
						   &old_allocation,
						   allocation);
}
...
static gboolean
gde2_panel_applet_frame_button_changed (GtkWidget      *widget,
					GdkEventButton *event)
{
	...
	case 3:
		if (event->type == GDK_BUTTON_PRESS ||
			...
			GDE2_PANEL_APPLET_FRAME_GET_CLASS (frame)->popup_menu (frame,
									  event->button,
									  event->time);
...
static void
gde2_panel_applet_frame_finalize (GObject *object)
{
	Gde2PanelAppletFrame *frame = GDE2_PANEL_APPLET_FRAME (object);

	gde2_panel_applets_manager_factory_deactivate (frame->priv->iid);

	panel_lockdown_notify_remove (G_CALLBACK (gde2_panel_applet_frame_sync_menu_state),
				      frame);

	g_free (frame->priv->iid);
	frame->priv->iid = NULL;

	G_OBJECT_CLASS (gde2_panel_applet_frame_parent_class)->finalize (object);
}

static void
gde2_panel_applet_frame_class_init (Gde2PanelAppletFrameClass *klass)
{
	GObjectClass   *gobject_class = (GObjectClass *) klass;
	GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;

	gobject_class->finalize = gde2_panel_applet_frame_finalize;

#if GTK_CHECK_VERSION (3, 0, 0)
	widget_class->draw                 = gde2_panel_applet_frame_draw;
	widget_class->get_preferred_width  = gde2_panel_applet_frame_get_preferred_width;
	widget_class->get_preferred_height = gde2_panel_applet_frame_get_preferred_height;
#else
	widget_class->expose_event         = gde2_panel_applet_frame_expose;
	widget_class->size_request         = gde2_panel_applet_frame_size_request;
#endif
	widget_class->size_allocate        = gde2_panel_applet_frame_size_allocate;
	widget_class->button_press_event   = gde2_panel_applet_frame_button_changed;
	widget_class->button_release_event = gde2_panel_applet_frame_button_changed;

	g_type_class_add_private (klass, sizeof (Gde2PanelAppletFramePrivate));
}

static void
gde2_panel_applet_frame_init (Gde2PanelAppletFrame *frame)
{
	frame->priv = GDE2_PANEL_APPLET_FRAME_GET_PRIVATE (frame);
...
static void
gde2_panel_applet_frame_init_properties (Gde2PanelAppletFrame *frame)
{
	GDE2_PANEL_APPLET_FRAME_GET_CLASS (frame)->init_properties (frame);
}

void
gde2_panel_applet_frame_sync_menu_state (Gde2PanelAppletFrame *frame)
{
	...
	movable = gde2_panel_applet_can_freely_move (frame->priv->applet_info);
	removable = panel_profile_id_lists_are_writable ();
	lockable = gde2_panel_applet_lockable (frame->priv->applet_info);

	locked = panel_widget_get_applet_locked (panel_widget, GTK_WIDGET (frame));
	locked_down = panel_lockdown_get_locked_down ();

	GDE2_PANEL_APPLET_FRAME_GET_CLASS (frame)->sync_menu_state (frame, movable, removable, lockable, locked, locked_down);
}

void
gde2_panel_applet_frame_change_orientation (Gde2PanelAppletFrame *frame,
				       PanelOrientation  orientation)
{
	...
	GDE2_PANEL_APPLET_FRAME_GET_CLASS (frame)->change_orientation (frame, orientation);
}

void
gde2_panel_applet_frame_change_size (Gde2PanelAppletFrame *frame,
				guint             size)
{
	GDE2_PANEL_APPLET_FRAME_GET_CLASS (frame)->change_size (frame, size);
}

void
gde2_panel_applet_frame_change_background (Gde2PanelAppletFrame    *frame,
				      PanelBackgroundType  type)
{
	...
	GDE2_PANEL_APPLET_FRAME_GET_CLASS (frame)->change_background (frame, type);
}

void
gde2_panel_applet_frame_set_panel (Gde2PanelAppletFrame *frame,
			      PanelWidget      *panel)
{
	g_return_if_fail (PANEL_IS_APPLET_FRAME (frame));
...
void
_gde2_panel_applet_frame_set_iid (Gde2PanelAppletFrame *frame,
			     const gchar      *iid)
{
	if (frame->priv->iid)
		g_free (frame->priv->iid);
...
void
_gde2_panel_applet_frame_activated (Gde2PanelAppletFrame           *frame,
			       ...
			       GError                     *error)
{
	...
	if (error != NULL) {
		...
		gde2_panel_applet_frame_loading_failed (frame->priv->iid,
						   frame_act->panel,
						   frame_act->id);
		gde2_panel_applet_frame_activating_free (frame_act);
	...
	info = gde2_panel_applet_register (GTK_WIDGET (frame), GTK_WIDGET (frame),
	...
	gde2_panel_applet_frame_sync_menu_state (frame);
	gde2_panel_applet_frame_init_properties (frame);

	panel_lockdown_notify_add (G_CALLBACK (gde2_panel_applet_frame_sync_menu_state),
				   frame);

	gde2_panel_applet_stop_loading (frame_act->id);
	gde2_panel_applet_frame_activating_free (frame_act);
}

void
_gde2_panel_applet_frame_update_flags (Gde2PanelAppletFrame *frame,
				  gboolean          major,
				  gboolean          minor,
				  gboolean          has_handle)
{
...
void
_gde2_panel_applet_frame_update_size_hints (Gde2PanelAppletFrame *frame,
				       gint             *size_hints,
				       guint             n_elements)
{
	if (frame->priv->has_handle) {
...
char *
_gde2_panel_applet_frame_get_background_string (Gde2PanelAppletFrame    *frame,
					   PanelWidget         *panel,
					   PanelBackgroundType  type)
{
	GtkAllocation allocation;
...
static void
gde2_panel_applet_frame_reload_response (GtkWidget        *dialog,
				    int               response,
				    Gde2PanelAppletFrame *frame)
{
	AppletInfo *info;
	...
	if (response == PANEL_RESPONSE_RELOAD) {
		...
		if (info) {
			id = g_strdup (info->id);
			position  = gde2_panel_applet_get_position (info);
			locked = panel_widget_get_applet_locked (panel, info->widget);
			gde2_panel_applet_clean (info);
		}

		gde2_panel_applet_frame_load (iid, panel, locked,
					 position, TRUE, id);
...
void
_gde2_panel_applet_frame_applet_broken (Gde2PanelAppletFrame *frame)
{
	GtkWidget  *dialog;
	GdkScreen  *screen;
	...
	if (frame->priv->iid) {
		...
		info = (Gde2PanelAppletInfo *)gde2_panel_applets_manager_get_applet_info (frame->priv->iid);
		applet_name = gde2_panel_applet_info_get_name (info);
	}
	...
	g_signal_connect (dialog, "response",
			  G_CALLBACK (gde2_panel_applet_frame_reload_response),
			  g_object_ref (frame));
...
void
_gde2_panel_applet_frame_applet_remove (Gde2PanelAppletFrame *frame)
{
	AppletInfo *info;
...
void
_gde2_panel_applet_frame_applet_move (Gde2PanelAppletFrame *frame)
{
...
void
_gde2_panel_applet_frame_applet_lock (Gde2PanelAppletFrame *frame,
				 gboolean          locked)
{
	...
	gde2_panel_applet_toggle_locked (frame->priv->applet_info);
}
...
static void
gde2_panel_applet_frame_activating_free (Gde2PanelAppletFrameActivating *frame_act)
{
	g_free (frame_act->id);
	g_slice_free (Gde2PanelAppletFrameActivating, frame_act);
}

GdkScreen *
panel_applet_frame_activating_get_screen (Gde2PanelAppletFrameActivating *frame_act)
{
    return gtk_widget_get_screen (GTK_WIDGET(frame_act->panel));
}

PanelOrientation
gde2_panel_applet_frame_activating_get_orientation(Gde2PanelAppletFrameActivating *frame_act)
{
	return panel_widget_get_applet_orientation(frame_act->panel);
}

guint32
gde2_panel_applet_frame_activating_get_size (Gde2PanelAppletFrameActivating *frame_act)
{
	return frame_act->panel->sz;
}

gboolean
gde2_panel_applet_frame_activating_get_locked (Gde2PanelAppletFrameActivating *frame_act)
{
	return frame_act->locked;
}

gboolean
gde2_panel_applet_frame_activating_get_locked_down (Gde2PanelAppletFrameActivating *frame_act)
{
	return panel_lockdown_get_locked_down ();
}

gchar *
gde2_panel_applet_frame_activating_get_conf_path (Gde2PanelAppletFrameActivating *frame_act)
{
	return g_strdup_printf (GDE2_PANEL_APPLET_PREFS_PATH, frame_act->id);
}

static void
gde2_panel_applet_frame_loading_failed_response (GtkWidget *dialog,
					    guint      response,
					    char      *id)
{
	gtk_widget_destroy (dialog);
...
static void
gde2_panel_applet_frame_loading_failed (const char  *iid,
				   PanelWidget *panel,
				   const char  *id)
{
	GtkWidget *dialog;
	...
	g_signal_connect (dialog, "response",
			  G_CALLBACK (gde2_panel_applet_frame_loading_failed_response),
			  g_strdup (id));
	...
	gde2_panel_applet_stop_loading (id);
}

static void
gde2_panel_applet_frame_load (const gchar *iid,
			 PanelWidget *panel,
			 gboolean     locked,
			 int          position,
			 gboolean     exactpos,
			 const char  *id)
{
	...
	frame_act = g_slice_new0 (Gde2PanelAppletFrameActivating);
	...
	if (!gde2_panel_applets_manager_load_applet (iid, frame_act)) {
		gde2_panel_applet_frame_loading_failed (iid, panel, id);
		gde2_panel_applet_frame_activating_free (frame_act);
	}
}

void
gde2_panel_applet_frame_load_from_gsettings (PanelWidget *panel_widget,
				    gboolean     locked,
				    int          position,
				    const char  *id)
{
	...
	gde2_panel_applet_frame_load (applet_iid, panel_widget,
...
void
gde2_panel_applet_frame_create (PanelToplevel *toplevel,
			   int            position,
			   const char    *iid)
{
	GSettings   *settings;
