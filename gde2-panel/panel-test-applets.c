...
#include <libgde2-panel-applet-private/panel-applet-container.h>
#include <libgde2-panel-applet-private/panel-applets-manager-dbus.h>
		     ...
		     GtkWidget    *applet_window)
{
	...
	if (!gde2_panel_applet_container_add_finish (GDE2_PANEL_APPLET_CONTAINER (source_object),
						res, &error)) {
		GtkWidget *dialog;
			 ...
			 guint       orientation)
{
	...
	gde2_panel_applet_container_add (GDE2_PANEL_APPLET_CONTAINER (container),
...
setup_options (void)
{
	Gde2PanelAppletsManager *manager;
	GList               *applet_list, *l;
	int                  i;
	int                  j;
	...
	applet_list = GDE2_PANEL_APPLETS_MANAGER_GET_CLASS (manager)->get_applets (manager);
	for (l = applet_list, i = 1; l; l = g_list_next (l), i++) {
		Gde2PanelAppletInfo *info = (Gde2PanelAppletInfo *)l->data;

		gtk_list_store_append (model, &iter);
		gtk_list_store_set (model, &iter,
				    COLUMN_TEXT, g_strdup (gde2_panel_applet_info_get_name (info)),
				    COLUMN_ITEM, g_strdup (gde2_panel_applet_info_get_iid (info)),
				    -1);
	}
	...
	{
		g_free (unique_key);
		unique_key = g_strdup_printf ("gde2-panel-test-applet-%d", i);
		unique_key_found = TRUE;
		dconf_paths = gde2_dconf_list_subdirs ("/tmp/", TRUE);
...
main (int argc, char **argv)
{
	...
	if (g_file_test ("../libgde2-panel-applet", G_FILE_TEST_IS_DIR)) {
		applets_dir = g_strdup_printf ("%s:../libgde2-panel-applet", GDE2_PANEL_APPLETS_DIR);
		g_setenv ("GDE2_PANEL_APPLETS_DIR", applets_dir, FALSE);
		g_free (applets_dir);
	}
