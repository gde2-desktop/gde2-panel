...
{
	static GtkTargetEntry target[] = {
		{ "application/x-gde2-panel-applet-iid", 0, 0 }
	};
...
panel_addto_query_applets (GSList *list)
{
	...
	applet_list = gde2_panel_applets_manager_get_applets ();
	...
	for (l = applet_list; l; l = g_list_next (l)) {
		...
		info = (Gde2PanelAppletInfo *)l->data;

		iid = gde2_panel_applet_info_get_iid (info);
		name = gde2_panel_applet_info_get_name (info);
		description = gde2_panel_applet_info_get_description (info);
		icon = gde2_panel_applet_info_get_icon (info);
			       ...
			       const char          *filename)
{
	...
	data->item_info.name          = g_strdup (gde2menu_tree_directory_get_name (directory));
	data->item_info.description   = g_strdup (gde2menu_tree_directory_get_comment (directory));
	data->item_info.icon          = g_strdup (gde2menu_tree_directory_get_icon (directory));
	data->item_info.menu_filename = g_strdup (filename);
	data->item_info.menu_path     = gde2menu_tree_directory_make_path (directory, NULL);
			   ...
			   const char      *filename)
{
	...
	data->item_info.name          = g_strdup (gde2menu_tree_entry_get_display_name (entry));
	data->item_info.description   = g_strdup (gde2menu_tree_entry_get_comment (entry));
	data->item_info.icon          = g_strdup (gde2menu_tree_entry_get_icon (entry));
	data->item_info.launcher_path = g_strdup (gde2menu_tree_entry_get_desktop_file_path (entry));
	data->item_info.static_data   = FALSE;
				   ...
				   const char          *filename)
{
	...
	for (l = items; l; l = l->next) {
		switch (gde2menu_tree_item_get_type (l->data)) {
		case GDE2MENU_TREE_ITEM_DIRECTORY:
		...
		gde2menu_tree_item_unref (l->data);
	}
...
static void panel_addto_make_application_model(PanelAddtoDialog* dialog)
{
	...
	tree = gde2menu_tree_lookup("gde2-applications.menu", GDE2MENU_TREE_FLAGS_NONE);
	gde2menu_tree_set_sort_key(tree, GDE2MENU_TREE_SORT_DISPLAY_NAME);

	if ((root = gde2menu_tree_get_root_directory (tree)))
	{
		panel_addto_make_application_list(&dialog->application_list, root, "gde2-applications.menu");
	...
	tree = gde2menu_tree_lookup("gde2-settings.menu", GDE2MENU_TREE_FLAGS_NONE);
	gde2menu_tree_set_sort_key(tree, GDE2MENU_TREE_SORT_DISPLAY_NAME);
	...
	if ((root = gde2menu_tree_get_root_directory(tree)))
	{
		...
		panel_addto_make_application_list(&dialog->settings_list, root, "gde2-settings.menu");
	 	      ...
	 	      PanelAddtoItemInfo *item_info)
{
	...
	case PANEL_ADDTO_APPLET:
		gde2_panel_applet_frame_create (dialog->panel_widget->toplevel,
			     ...
			     PanelAddtoDialog *dialog)
{
	...
	case GTK_RESPONSE_HELP:
		panel_show_help (gtk_window_get_screen (GTK_WINDOW (dialog->addto_dialog)),
				 "gde2-user-guide", "gospanel-15", NULL);
		break;
...
panel_addto_dialog_free (PanelAddtoDialog *dialog)
{
	...
	if (dialog->menu_tree)
		gde2menu_tree_unref (dialog->menu_tree);
