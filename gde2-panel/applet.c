/* Gde2 panel: general applet functionality
...
gde2_panel_applet_get_panel_widget (AppletInfo *info)
{
	return PANEL_WIDGET (gtk_widget_get_parent (info->widget));
}

static void
gde2_panel_applet_set_dnd_enabled (AppletInfo *info,
...
gboolean
gde2_panel_applet_toggle_locked (AppletInfo *info)
{
	PanelWidget *panel_widget;
	gboolean     locked;

	panel_widget = gde2_panel_applet_get_panel_widget (info);

	locked = panel_widget_toggle_applet_locked (panel_widget, info->widget);

	gde2_panel_applet_save_position (info, info->id, TRUE);
	gde2_panel_applet_set_dnd_enabled (info, !locked);

	return locked;
}

static void
gde2_panel_applet_lock (GtkCheckMenuItem *menuitem,
		   AppletInfo       *info)
{
	gboolean locked;

	locked = gde2_panel_applet_toggle_locked (info);
 ...
 * cleanups should go in gde2_panel_applet_destroy()
 */
void
gde2_panel_applet_clean (AppletInfo *info)
{
	g_return_if_fail (info != NULL);
...
static void
gde2_panel_applet_recreate_menu (AppletInfo	*info)
{
	GList *l;
	...
	info->menu = gde2_panel_applet_create_menu (info);
}

static void
gde2_panel_applet_locked_change_notify (GSettings *settings,
									    gchar *key,
									    GtkWidget   *applet)
{
	gboolean     locked;
	...
	gde2_panel_applet_toggle_locked (info);

	if (info->type == PANEL_OBJECT_APPLET)
		gde2_panel_applet_frame_sync_menu_state (GDE2_PANEL_APPLET_FRAME (info->widget));
	else
		gde2_panel_applet_recreate_menu (info);
}
...
applet_user_menu_get_screen (AppletUserMenu *menu)
{
	...
	panel_widget = gde2_panel_applet_get_panel_widget (menu->info);
	...
	case PANEL_OBJECT_DRAWER:
		if (strcmp (menu->name, "add") == 0) {
			...
			panel_show_help (screen,
					 "gde2-user-guide", "gospanel-18", NULL);
		}
		break;
	case PANEL_OBJECT_MENU:
...
AppletUserMenu *
gde2_panel_applet_get_callback (GList      *user_menu,
			   const char *name)
{
	GList *l;
...
void
gde2_panel_applet_add_callback (AppletInfo          *info,
			   ...
			   CallbackEnabledFunc  is_enabled_func)
{
	...
	g_return_if_fail (info != NULL);
	g_return_if_fail (gde2_panel_applet_get_callback (info->user_menu,
						     callback_name) == NULL);
		 ...
		 GList *user_menu)
{
	...
	*p = '\0';
	...
	s_menu = gde2_panel_applet_get_callback (user_menu, t);
...
GtkWidget *
gde2_panel_applet_create_menu (AppletInfo *info)
{
	GtkWidget   *menu;
	GtkWidget   *menuitem;
	GList       *l;
	...
	if (!panel_lockdown_get_locked_down ()) {
		...
		lockable = gde2_panel_applet_lockable (info);
		movable = gde2_panel_applet_can_freely_move (info);
		...
		g_signal_connect (menuitem, "toggled",
				  G_CALLBACK (gde2_panel_applet_lock), info);
		gtk_widget_show (menuitem);
		gtk_menu_shell_append (GTK_MENU_SHELL (menu), menuitem);
...
void
gde2_panel_applet_menu_set_recurse (GtkMenu     *menu,
			       const gchar *key,
			       gpointer     data)
{
	GList *children;
	...
	for (l = children; l; l = l->next) {
		...
		if (submenu)
			gde2_panel_applet_menu_set_recurse (
		  ...
		  GdkEventButton *event)
{
	...
	gde2_panel_applet_menu_set_recurse (GTK_MENU (info->menu),
				       "menu_panel",
				       panel_widget);
...
applet_do_popup_menu (GtkWidget      *widget,
		      ...
		      AppletInfo     *info)
{
	if (gde2_panel_applet_is_in_drag ())
		return FALSE;
...
static void
gde2_panel_applet_destroy (GtkWidget  *widget,
		      AppletInfo *info)
{
	GList *l;
	...
	if (info->type != PANEL_OBJECT_APPLET)
		panel_lockdown_notify_remove (G_CALLBACK (gde2_panel_applet_recreate_menu),
					      info);
...
typedef struct {
	...
	guint            right_stick : 1;
	guint            locked : 1;
} Gde2PanelAppletToLoad;

/* Each time those lists get both empty,
 * gde2_panel_applet_queue_initial_unhide_toplevels() should be called */
static GSList  *gde2_panel_applets_to_load = NULL;
static GSList  *gde2_panel_applets_loading = NULL;
/* We have a timeout to always unhide toplevels after a delay, in case of some
 * blocking applet */
#define         UNHIDE_TOPLEVELS_TIMEOUT_SECONDS 5
static guint    gde2_panel_applet_unhide_toplevels_timeout = 0;

static gboolean gde2_panel_applet_have_load_idle = FALSE;

static void
free_applet_to_load (Gde2PanelAppletToLoad *applet)
{
	g_free (applet->id);
	applet->id = NULL;
...
gboolean
gde2_panel_applet_on_load_queue (const char *id)
{
	GSList *li;
	for (li = gde2_panel_applets_to_load; li != NULL; li = li->next) {
		Gde2PanelAppletToLoad *applet = li->data;
		if (strcmp (applet->id, id) == 0)
			return TRUE;
	}
	for (li = gde2_panel_applets_loading; li != NULL; li = li->next) {
		Gde2PanelAppletToLoad *applet = li->data;
		if (strcmp (applet->id, id) == 0)
			return TRUE;
	}
...
static gboolean
gde2_panel_applet_queue_initial_unhide_toplevels (gpointer user_data)
{
	GSList *l;

	if (gde2_panel_applet_unhide_toplevels_timeout != 0) {
		g_source_remove (gde2_panel_applet_unhide_toplevels_timeout);
		gde2_panel_applet_unhide_toplevels_timeout = 0;
	}
...
void
gde2_panel_applet_stop_loading (const char *id)
{
	Gde2PanelAppletToLoad *applet;
	GSList *l;

	for (l = gde2_panel_applets_loading; l; l = l->next) {
		applet = l->data;
	 ...
	 * for example */
	if (l != NULL) {
		gde2_panel_applets_loading = g_slist_delete_link (gde2_panel_applets_loading, l);
		free_applet_to_load (applet);
	}

	if (gde2_panel_applets_loading == NULL && gde2_panel_applets_to_load == NULL)
		gde2_panel_applet_queue_initial_unhide_toplevels (NULL);
}

static gboolean
gde2_panel_applet_load_idle_handler (gpointer dummy)
{
	PanelObjectType    applet_type;
	Gde2PanelAppletToLoad *applet = NULL;
	PanelToplevel     *toplevel = NULL;
	PanelWidget       *panel_widget;
	GSList            *l;

	if (!gde2_panel_applets_to_load) {
		gde2_panel_applet_have_load_idle = FALSE;
		return FALSE;
	}

	for (l = gde2_panel_applets_to_load; l; l = l->next) {
		applet = l->data;
	...
	if (!l) {
		...
		g_slist_free (gde2_panel_applets_to_load);
		gde2_panel_applets_to_load = NULL;
		gde2_panel_applet_have_load_idle = FALSE;

		if (gde2_panel_applets_loading == NULL) {
	...
	gde2_panel_applets_to_load = g_slist_delete_link (gde2_panel_applets_to_load, l);
	gde2_panel_applets_loading = g_slist_append (gde2_panel_applets_loading, applet);
	 ...
	 * gde2_panel_applet_stop_loading() for this type. However, in case of
	 * failure during the load, we might call gde2_panel_applet_stop_loading()
	...
	switch (applet_type) {
	case PANEL_OBJECT_APPLET:
		gde2_panel_applet_frame_load_from_gsettings (
					panel_widget,
					applet->locked,
					applet->position,
					applet->id);
	...
	if (applet_type != PANEL_OBJECT_APPLET)
		gde2_panel_applet_stop_loading (applet->id);

	return TRUE;
}

void
gde2_panel_applet_queue_applet_to_load (const char      *id,
				   PanelObjectType  type,
				   ...
				   gboolean         locked)
{
	...
	applet = g_new0 (Gde2PanelAppletToLoad, 1);
	...
	gde2_panel_applets_to_load = g_slist_prepend (gde2_panel_applets_to_load, applet);
}

static int
gde2_panel_applet_compare (const Gde2PanelAppletToLoad *a,
		      const Gde2PanelAppletToLoad *b)
{
	int c;
...
void
gde2_panel_applet_load_queued_applets (gboolean initial_load)
{
	if (!gde2_panel_applets_to_load) {
		gde2_panel_applet_queue_initial_unhide_toplevels (NULL);
		return;
	...
	if (gde2_panel_applets_to_load && gde2_panel_applet_unhide_toplevels_timeout == 0) {
		/* Install a timeout to make sure we don't block the
		 * unhiding because of an applet that doesn't load */
		gde2_panel_applet_unhide_toplevels_timeout =
			g_timeout_add_seconds (UNHIDE_TOPLEVELS_TIMEOUT_SECONDS,
					       gde2_panel_applet_queue_initial_unhide_toplevels,
					       NULL);
	}

	gde2_panel_applets_to_load = g_slist_sort (gde2_panel_applets_to_load,
					      (GCompareFunc) gde2_panel_applet_compare);

	if ( ! gde2_panel_applet_have_load_idle) {
		...
		if (initial_load)
			g_idle_add_full (G_PRIORITY_HIGH_IDLE,
					 gde2_panel_applet_load_idle_handler,
					 NULL, NULL);
		else
			g_idle_add (gde2_panel_applet_load_idle_handler, NULL);

		gde2_panel_applet_have_load_idle = TRUE;
	}
}

static const char* gde2_panel_applet_get_toplevel_id(AppletInfo* applet)
{
	PanelWidget* panel_widget;
	...
	panel_widget = gde2_panel_applet_get_panel_widget(applet);
...
static gboolean
gde2_panel_applet_position_save_timeout (gpointer dummy)
{
	GSList *l;
...
void
gde2_panel_applet_save_position (AppletInfo *applet_info,
			    const char *id,
			    gboolean    immediate)
{
	PanelWidget       *panel_widget;
		...
		if (!queued_position_source)
			queued_position_source =
				g_timeout_add_seconds (1,
						       (GSourceFunc) gde2_panel_applet_position_save_timeout,
						       NULL);
	...
	if (!(toplevel_id = gde2_panel_applet_get_toplevel_id (applet_info)))
		return;

	panel_widget = gde2_panel_applet_get_panel_widget (applet_info);
	...
	position = gde2_panel_applet_get_position (applet_info);
...
const char *
gde2_panel_applet_get_id (AppletInfo *info)
{
	if (!info)
		return NULL;
...
const char *
gde2_panel_applet_get_id_by_widget (GtkWidget *applet_widget)
{
	GSList *l;
...
AppletInfo *
gde2_panel_applet_get_by_id (const char *id)
{
	GSList *l;
...
GSList *
gde2_panel_applet_list_applets (void)
{
	return registered_applets;
}

AppletInfo *
gde2_panel_applet_get_by_type (PanelObjectType object_type, GdkScreen *screen)
{
	GSList *l;
...
AppletInfo *
gde2_panel_applet_register (GtkWidget       *applet,
		       gpointer         data,
		       ...
		       const char      *id)
{
	...
	if (type != PANEL_OBJECT_APPLET)
		panel_lockdown_notify_add (G_CALLBACK (gde2_panel_applet_recreate_menu),
					   info);

	locked_changed = g_strdup_printf ("changed::%s", PANEL_OBJECT_LOCKED_KEY);
	g_signal_connect (info->settings,
					  locked_changed,
					  G_CALLBACK (gde2_panel_applet_locked_change_notify),
					  G_OBJECT (applet));
	...
	if (type == PANEL_OBJECT_DRAWER) {
		...
		g_object_set_data (G_OBJECT (applet),
				   GDE2_PANEL_APPLET_ASSOC_PANEL_KEY, assoc_panel);
	...
	g_object_set_data (G_OBJECT (applet),
			   GDE2_PANEL_APPLET_FORBIDDEN_PANELS, NULL);
	...
	g_signal_connect (applet, "destroy",
			  G_CALLBACK (gde2_panel_applet_destroy),
			  info);
...
int
gde2_panel_applet_get_position (AppletInfo *applet)
{
	...
	applet_data = g_object_get_data (G_OBJECT (applet->widget), GDE2_PANEL_APPLET_DATA);

	return applet_data->pos;
}

gboolean
gde2_panel_applet_can_freely_move (AppletInfo *applet)
{
	if (panel_lockdown_get_locked_down ())
		return FALSE;
...
gboolean
gde2_panel_applet_lockable (AppletInfo *applet)
{
	if (panel_lockdown_get_locked_down ())
		return FALSE;
