...
static char* panel_lock_screen_action_get_command(const char* action)
{
	...
	if (panel_is_program_in_path("gde2-screensaver-command") && panel_is_program_in_path("gde2-screensaver-preferences"))
	{
	...
	if (strcmp (action, "prefs") == 0)
	{
		if (use_gscreensaver)
		{
			command = g_strdup ("gde2-screensaver-preferences");
		}
	...
	{
		/* Neither gde2-screensaver or xscreensaver allow root
		 * to lock the screen */
		if (geteuid () == 0)
		{
			command = NULL;
		...
		else
		{
			if (use_gscreensaver)
			{
				command = g_strdup_printf("gde2-screensaver-command --%s", action);
			}
			else
			{
...
void panel_lock_screen_action(GdkScreen* screen, const char* action)
{
	...
	if (!gde2_gdk_spawn_command_line_on_screen(screen, command, &error))
	{
...
{
		return g_build_filename(g_get_user_config_dir(), "gde2", "panel2.d", "default", "launchers", NULL);
}
...
panel_util_get_file_display_for_common_files (GFile *file)
{
	...
	if (g_file_equal (file, compare)) {
		...
		if (gde2_gsettings_schema_exists (CAJA_DESKTOP_SCHEMA)) {
...
panel_util_get_icon_for_uri (const char *text_uri)
{
	 ...
	 *  + check for application/x-gde2-saved-search mime type and override
	 *    icon of the GFile
