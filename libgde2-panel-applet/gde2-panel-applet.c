/*
 * gde2-panel-applet.c: panel applet writing library.
 *
...
#include "gde2-panel-applet-marshal.h"
#include "gde2-panel-applet-enums.h"
#if GTK_CHECK_VERSION (3, 18, 0)
#include "panel-plug-private.h"
#endif
#define GDE2_PANEL_APPLET_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), PANEL_TYPE_APPLET, Gde2PanelAppletPrivate))

struct _Gde2PanelAppletPrivate {
	GtkWidget         *plug;
	GtkWidget         *applet;
...
static guint gde2_panel_applet_signals[LAST_SIGNAL] = { 0 };
...
static void       gde2_panel_applet_handle_background   (Gde2PanelApplet       *applet);
static GtkAction *gde2_panel_applet_menu_get_action     (Gde2PanelApplet       *applet,
						    const gchar       *action);
static void       gde2_panel_applet_menu_update_actions (Gde2PanelApplet       *applet);
static void       gde2_panel_applet_menu_cmd_remove     (GtkAction         *action,
						    Gde2PanelApplet       *applet);
static void       gde2_panel_applet_menu_cmd_move       (GtkAction         *action,
						    Gde2PanelApplet       *applet);
static void       gde2_panel_applet_menu_cmd_lock       (GtkAction         *action,
						    Gde2PanelApplet       *applet);
static void       gde2_panel_applet_register_object     (Gde2PanelApplet       *applet);
#if GTK_CHECK_VERSION (3, 0, 0)
void	_gde2_panel_applet_apply_css	(GtkWidget* widget, Gde2PanelAppletBackgroundType type);
#endif

static const gchar panel_menu_ui[] =
	"<ui>\n"
	"  <popup name=\"Gde2PanelAppletPopup\" action=\"PopupAction\">\n"
...
static const GtkActionEntry menu_entries[] = {
	{ "Remove", GTK_STOCK_REMOVE, N_("_Remove From Panel"),
	  NULL, NULL,
	  G_CALLBACK (gde2_panel_applet_menu_cmd_remove) },
	{ "Move", NULL, N_("_Move"),
	  NULL, NULL,
	  G_CALLBACK (gde2_panel_applet_menu_cmd_move) }
};

static const GtkToggleActionEntry menu_toggle_entries[] = {
	{ "Lock", NULL, N_("Loc_k To Panel"),
	  NULL, NULL,
	  G_CALLBACK (gde2_panel_applet_menu_cmd_lock) }
};

G_DEFINE_TYPE (Gde2PanelApplet, gde2_panel_applet, GTK_TYPE_EVENT_BOX)

#define GDE2_PANEL_APPLET_INTERFACE   "org.gde2.panel.applet.Applet"
#define GDE2_PANEL_APPLET_OBJECT_PATH "/org/gde2/panel/applet/%s/%d"

char *
gde2_panel_applet_get_preferences_path (Gde2PanelApplet *applet)
{
...
static void
gde2_panel_applet_set_preferences_path (Gde2PanelApplet *applet,
...
gde2_panel_applet_get_flags (Gde2PanelApplet *applet)
{
	g_return_val_if_fail (PANEL_IS_APPLET (applet), GDE2_PANEL_APPLET_FLAGS_NONE);

	return applet->priv->flags;
}

void
gde2_panel_applet_set_flags (Gde2PanelApplet      *applet,
			Gde2PanelAppletFlags  flags)
{
...
static void
gde2_panel_applet_size_hints_ensure (Gde2PanelApplet *applet,
				int          new_size)
{
...
static gboolean
gde2_panel_applet_size_hints_changed (Gde2PanelApplet *applet,
...
void
gde2_panel_applet_set_size_hints (Gde2PanelApplet *applet,
			     const int   *size_hints,
			     int          n_elements,
			     int          base_size)
{
	...
	if (!gde2_panel_applet_size_hints_changed (applet, size_hints, n_elements, base_size))
		return;

	gde2_panel_applet_size_hints_ensure (applet, n_elements);
	for (i = 0; i < n_elements; i++)
...
guint
gde2_panel_applet_get_size (Gde2PanelApplet *applet)
{
...
static void
gde2_panel_applet_set_size (Gde2PanelApplet *applet,
		       guint        size)
{
	g_return_if_fail (PANEL_IS_APPLET (applet));
	...
	g_signal_emit (G_OBJECT (applet),
		       gde2_panel_applet_signals [CHANGE_SIZE],
		       0, size);
...
gde2_panel_applet_get_orient (Gde2PanelApplet *applet)
{
	g_return_val_if_fail (PANEL_IS_APPLET (applet), 0);
...
static void
gde2_panel_applet_set_orient (Gde2PanelApplet      *applet,
			 Gde2PanelAppletOrient orient)
{
	g_return_if_fail (PANEL_IS_APPLET (applet));
	...
	g_signal_emit (G_OBJECT (applet),
		       gde2_panel_applet_signals [CHANGE_ORIENT],
		       0, orient);
...
static void
gde2_panel_applet_set_locked (Gde2PanelApplet *applet,
			 gboolean     locked)
{
	GtkAction *action;
	...
	action = gde2_panel_applet_menu_get_action (applet, "Lock");
	g_signal_handlers_block_by_func (action,
					 gde2_panel_applet_menu_cmd_lock,
					 applet);
	...
	gde2_panel_applet_menu_update_actions (applet);
...
gboolean
gde2_panel_applet_get_locked_down (Gde2PanelApplet *applet)
{
...
static void
gde2_panel_applet_set_locked_down (Gde2PanelApplet *applet,
			      gboolean     locked_down)
{
	g_return_if_fail (PANEL_IS_APPLET (applet));
...
static void
gde2_panel_applet_init_atoms (Display *xdisplay)
{
...
static Window
gde2_panel_applet_find_toplevel_dock_window (Gde2PanelApplet *applet,
					Display	    *xdisplay)
{
	GtkWidget  *toplevel;
	Window	    xwin;
 ...
 */
void
gde2_panel_applet_request_focus (Gde2PanelApplet	 *applet,
			    guint32	  timestamp)
{
	GdkScreen  *screen;
	GdkWindow  *root;
	...
	gde2_panel_applet_init_atoms (xdisplay);

	dock_xwindow = gde2_panel_applet_find_toplevel_dock_window (applet, xdisplay);
...
static GtkAction *
gde2_panel_applet_menu_get_action (Gde2PanelApplet *applet,
			      const gchar *action)
{
...
static void
gde2_panel_applet_menu_update_actions (Gde2PanelApplet *applet)
{
	gboolean locked = applet->priv->locked;
	gboolean locked_down = applet->priv->locked_down;

	g_object_set (gde2_panel_applet_menu_get_action (applet, "Lock"),
		      "visible", !locked_down, NULL);
	g_object_set (gde2_panel_applet_menu_get_action (applet, "Move"),
		      "sensitive", !locked,
		      "visible", !locked_down,
		      NULL);
	g_object_set (gde2_panel_applet_menu_get_action (applet, "Remove"),
		      "sensitive", !locked,
		      "visible", !locked_down,
		      NULL);
}
...
static void
gde2_panel_applet_menu_cmd_remove (GtkAction   *action,
			      Gde2PanelApplet *applet)
{
	GError *error = NULL;
...
static void
gde2_panel_applet_menu_cmd_move (GtkAction   *action,
			    Gde2PanelApplet *applet)
{
	GError *error = NULL;
...
static void
gde2_panel_applet_menu_cmd_lock (GtkAction   *action,
			    Gde2PanelApplet *applet)
{
	gboolean locked;

	locked = gtk_toggle_action_get_active (GTK_TOGGLE_ACTION (action));
	gde2_panel_applet_set_locked (applet, locked);
}

void
gde2_panel_applet_setup_menu (Gde2PanelApplet    *applet,
			 const gchar    *xml,
			 GtkActionGroup *applet_action_group)
{
	gchar  *new_xml;
	...
	new_xml = g_strdup_printf ("<ui><popup name=\"Gde2PanelAppletPopup\" action=\"AppletItems\">"
...
void
gde2_panel_applet_setup_menu_from_file (Gde2PanelApplet    *applet,
				   ...
				   GtkActionGroup *applet_action_group)
{
	...
	if (g_file_get_contents (filename, &xml, NULL, &error)) {
		gde2_panel_applet_setup_menu (applet, xml, applet_action_group);
	} else {
...
static void
gde2_panel_applet_finalize (GObject *object)
{
	Gde2PanelApplet *applet = GDE2_PANEL_APPLET (object);
	...
	gde2_panel_applet_set_preferences_path (applet, NULL);
	...
	G_OBJECT_CLASS (gde2_panel_applet_parent_class)->finalize (object);
}
...
static void
gde2_panel_applet_position_menu (GtkMenu   *menu,
			    int       *x,
			    int       *y,
			    gboolean  *push_in,
			    GtkWidget *widget)
{
	...
	applet = GDE2_PANEL_APPLET (widget);
	...
	if (applet->priv->orient == GDE2_PANEL_APPLET_ORIENT_UP ||
	    applet->priv->orient == GDE2_PANEL_APPLET_ORIENT_DOWN) {
...
static void
gde2_panel_applet_menu_popup (Gde2PanelApplet *applet,
			 guint        button,
			 guint32      time)
{
	GtkWidget *menu;
...
/* Set menu and it's toplevel window to follow panel theme */
	...
	gtk_style_context_add_class(context,"gde2-panel-menu-bar");
#endif					  
	gtk_menu_popup (GTK_MENU (menu),
			NULL, NULL,
			(GtkMenuPositionFunc) gde2_panel_applet_position_menu,
			applet,
			button, time);
}

static gboolean
gde2_panel_applet_can_focus (GtkWidget *widget)
{
	/*
	 * A Gde2PanelApplet widget can focus if it has a tooltip or it does
...
/* Taken from libgde2componentui/gde2component/gde2component-plug.c */
static gboolean
gde2_panel_applet_button_event (GtkWidget      *widget,
...
static gboolean
gde2_panel_applet_button_press (GtkWidget      *widget,
			   GdkEventButton *event)
{
	Gde2PanelApplet *applet = GDE2_PANEL_APPLET (widget);
	...
	if (event->button == 3) {
		gde2_panel_applet_menu_popup (applet, event->button, event->time);

		return TRUE;
	}

	return gde2_panel_applet_button_event (applet->priv->plug, event);
}

static gboolean
gde2_panel_applet_button_release (GtkWidget      *widget,
			     GdkEventButton *event)
{
	Gde2PanelApplet *applet = GDE2_PANEL_APPLET (widget);
...
static gboolean
gde2_panel_applet_popup_menu (GtkWidget *widget)
{
	gde2_panel_applet_menu_popup (GDE2_PANEL_APPLET (widget), 3, GDK_CURRENT_TIME);
...
#if !GTK_CHECK_VERSION (3, 18, 0)
static void
gde2_panel_applet_get_preferred_width (GtkWidget *widget,
				       int       *minimum_width,
				       int       *natural_width)
{
	int focus_width = 0;

	GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->get_preferred_width (widget,
										minimum_width,
										natural_width);
	if (!gde2_panel_applet_can_focus (widget))
		return;
...
static void
gde2_panel_applet_get_preferred_height (GtkWidget *widget,
					int       *minimum_height,
					int       *natural_height)
{
	int focus_width = 0;

	GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->get_preferred_height (widget,
										 minimum_height,
										 natural_height);
	if (!gde2_panel_applet_can_focus (widget))
		return;
...
gde2_panel_applet_get_request_mode (GtkWidget *widget)
{
	Gde2PanelApplet *applet = GDE2_PANEL_APPLET (widget);
	Gde2PanelAppletOrient orientation;

	orientation = gde2_panel_applet_get_orient (applet);
	if (orientation == GDE2_PANEL_APPLET_ORIENT_UP ||
	    orientation == GDE2_PANEL_APPLET_ORIENT_DOWN)
...
#else
static void
gde2_panel_applet_size_request (GtkWidget *widget, GtkRequisition *requisition)
{
	int focus_width = 0;

	GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->size_request (widget,
									 requisition);
...
static void
gde2_panel_applet_size_allocate (GtkWidget     *widget,
			    GtkAllocation *allocation)
{
	...
	if (!gde2_panel_applet_can_focus (widget)) {
		GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->size_allocate (widget, allocation);
	} else {
#if !GTK_CHECK_VERSION (3, 18, 0)
		/*
		 * We are deliberately ignoring focus-padding here to
	...
	if (applet->priv->previous_height != allocation->height ||
		...
		gde2_panel_applet_handle_background (applet);
	}
}

#if GTK_CHECK_VERSION (3, 0, 0)
static gboolean gde2_panel_applet_draw(GtkWidget* widget, cairo_t* cr)
#else
static gboolean gde2_panel_applet_expose(GtkWidget* widget, GdkEventExpose* event)
#endif
{
#if GTK_CHECK_VERSION (3, 0, 0)
	GtkStyleContext *context;
...
#if GTK_CHECK_VERSION (3, 0, 0)
	GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->draw(widget, cr);
#else
	GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->expose_event(widget, event);
#endif
...
static gboolean
gde2_panel_applet_focus (GtkWidget        *widget,
		    GtkDirectionType  dir)
{
	...
	ret = GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->focus (widget, dir);
...
static gboolean
gde2_panel_applet_parse_color (const gchar *color_str,
#if GTK_CHECK_VERSION (3, 0, 0)
			       GdkRGBA     *color)
#else
			       GdkColor    *color)
...
static gboolean
gde2_panel_applet_parse_pixmap_str (const char *str,
#if GTK_CHECK_VERSION (3, 0, 0)
			       Window          *xid,
#else
			       GdkNativeWindow *xid,
...
gde2_panel_applet_create_foreign_surface_for_display (GdkDisplay *display,
                                                      GdkVisual  *visual,
                                                      Window      xid)
{
	Status result = 0;
...
gde2_panel_applet_get_pattern_from_pixmap (Gde2PanelApplet *applet,
#else
static GdkPixmap *
gde2_panel_applet_get_pixmap (Gde2PanelApplet     *applet,
#endif
#if GTK_CHECK_VERSION (3, 0, 0)
			 Window           xid,
#else
...
#if GTK_CHECK_VERSION (3, 0, 0)
	background = gde2_panel_applet_create_foreign_surface_for_display (gdk_window_get_display (window),
...
static Gde2PanelAppletBackgroundType
gde2_panel_applet_handle_background_string (Gde2PanelApplet  *applet,
#if GTK_CHECK_VERSION (3, 0, 0)
					    GdkRGBA          *color,
					    cairo_pattern_t **pattern)
#else
...
#endif
{
	Gde2PanelAppletBackgroundType   retval;
	char                          **elements;
	...
	if (elements [0] && !strcmp (elements [0], "none" )) {
		...
		if (!elements [1] || !gde2_panel_applet_parse_color (elements [1], color)) {
		...
		if (!gde2_panel_applet_parse_pixmap_str (elements [1], &pixmap_id, &x, &y)) {
...
#if GTK_CHECK_VERSION (3, 0, 0)
		*pattern = gde2_panel_applet_get_pattern_from_pixmap (applet, pixmap_id, x, y);
		if (!*pattern) {
...
#else
		*pixmap = gde2_panel_applet_get_pixmap (applet, pixmap_id, x, y);
		if (!*pixmap) {
...
gde2_panel_applet_get_background (Gde2PanelApplet  *applet,
#if GTK_CHECK_VERSION (3, 0, 0)
				  GdkRGBA          *color,
				  cairo_pattern_t **pattern)
#else
...
#if GTK_CHECK_VERSION (3, 0, 0)
	return gde2_panel_applet_handle_background_string (applet, color, pattern);
#else
	return gde2_panel_applet_handle_background_string (applet, color, pixmap);
#endif
}

static void
gde2_panel_applet_set_background_string (Gde2PanelApplet *applet,
...
#if !GTK_CHECK_VERSION (3, 0, 0)
static void
gde2_panel_applet_update_background_for_widget (GtkWidget                 *widget,
...
#if GTK_CHECK_VERSION (3, 0, 0)
static void
gde2_panel_applet_handle_background (Gde2PanelApplet *applet)
{
	Gde2PanelAppletBackgroundType  type;
	...
	type = gde2_panel_applet_get_background (applet, &color, &pattern);

	switch (type) {
	case PANEL_NO_BACKGROUND:
		g_signal_emit (G_OBJECT (applet),
			       gde2_panel_applet_signals [CHANGE_BACKGROUND],
...
gde2_panel_applet_handle_background (Gde2PanelApplet *applet)
{
	...
	type = gde2_panel_applet_get_background (applet, &color, &pixmap);

	if (applet->priv->background_widget)
		gde2_panel_applet_update_background_for_widget (applet->priv->background_widget,
							   type, &color, pixmap);
...
static void
gde2_panel_applet_realize (GtkWidget *widget)
{
	GTK_WIDGET_CLASS (gde2_panel_applet_parent_class)->realize (widget);

	if (GDE2_PANEL_APPLET (widget)->priv->background)
		gde2_panel_applet_handle_background (GDE2_PANEL_APPLET (widget));
}

static void
gde2_panel_applet_move_focus_out_of_applet (Gde2PanelApplet      *applet,
				       GtkDirectionType  dir)
{
	GtkWidget *toplevel;
...
#if GTK_CHECK_VERSION (3, 18, 0)
static void
gde2_panel_applet_change_background(Gde2PanelApplet *applet,
				    Gde2PanelAppletBackgroundType type,
				    GdkRGBA* color,
				    cairo_pattern_t *pattern)
{
	...
	_gde2_panel_applet_apply_css(GTK_WIDGET(applet->priv->plug),type);
	switch (type) {
	case PANEL_NO_BACKGROUND:
...
static void
gde2_panel_applet_get_property (GObject    *object,
			   guint       prop_id,
			   GValue     *value,
			   GParamSpec *pspec)
{
...
static void
gde2_panel_applet_set_property (GObject      *object,
			   guint         prop_id,
			   const GValue *value,
			   GParamSpec   *pspec)
{
	...
	case PROP_CLOSURE:
		...
		g_closure_set_marshal (applet->priv->closure,
				       gde2_panel_applet_marshal_BOOLEAN__STRING);
		break;
	case PROP_CONNECTION:
		applet->priv->connection = g_value_dup_object (value);
		break;
	case PROP_PREFS_PATH:
		gde2_panel_applet_set_preferences_path (applet, g_value_get_string (value));
		break;
	case PROP_ORIENT:
		gde2_panel_applet_set_orient (applet, g_value_get_uint (value));
		break;
	case PROP_SIZE:
		gde2_panel_applet_set_size (applet, g_value_get_uint (value));
		break;
	case PROP_BACKGROUND:
		gde2_panel_applet_set_background_string (applet, g_value_get_string (value));
		break;
	case PROP_FLAGS:
		gde2_panel_applet_set_flags (applet, g_value_get_uint (value));
		break;
	case PROP_SIZE_HINTS: {
		const int *size_hints;
		gsize      n_elements;
		...
		gde2_panel_applet_set_size_hints (applet, size_hints, n_elements, 0);
	}
		break;
	case PROP_LOCKED:
		gde2_panel_applet_set_locked (applet, g_value_get_boolean (value));
		break;
	case PROP_LOCKED_DOWN:
		gde2_panel_applet_set_locked_down (applet, g_value_get_boolean (value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
	}
...
static void
gde2_panel_applet_setup (Gde2PanelApplet *applet)
{
	GValue   value = {0, };
	GArray  *params;
	gint     i;
...
void _gde2_panel_applet_apply_css(GtkWidget* widget, Gde2PanelAppletBackgroundType type)
{
	...
	switch (type) {
	case PANEL_NO_BACKGROUND:
		gtk_style_context_remove_class (context, "gde2-custom-panel-background");
		break;
	case PANEL_COLOR_BACKGROUND:
	case PANEL_PIXMAP_BACKGROUND:
		gtk_style_context_add_class (context, "gde2-custom-panel-background");
		break;
	default:
		g_assert_not_reached ();
		break;
...
static void _gde2_panel_applet_prepare_css (GtkStyleContext *context)
{
	GtkCssProvider  *provider;

	provider = gtk_css_provider_new ();
	gtk_css_provider_load_from_data (provider,
					 ".gde2-custom-panel-background{\n"
...
static void
gde2_panel_applet_init (Gde2PanelApplet *applet)
{
	applet->priv = GDE2_PANEL_APPLET_GET_PRIVATE (applet);

	applet->priv->flags  = GDE2_PANEL_APPLET_FLAGS_NONE;
	applet->priv->orient = GDE2_PANEL_APPLET_ORIENT_UP;
	applet->priv->size   = 24;
	...
	_gde2_panel_applet_prepare_css(context);
#endif
	g_signal_connect_swapped (G_OBJECT (applet->priv->plug), "embedded",
				  G_CALLBACK (gde2_panel_applet_setup),
				  applet);
...
static void
gde2_panel_applet_constructed (GObject* object)
{
	Gde2PanelApplet* applet = GDE2_PANEL_APPLET(object);

	/* Rename the class to have compatibility with all GTK2 themes
	 * https://github.com/perberos/Gde2-Desktop-Environment/issues/27
	 */
	gtk_widget_set_name(GTK_WIDGET(applet), "PanelApplet");

	gde2_panel_applet_register_object (applet);
}

static void
gde2_panel_applet_class_init (Gde2PanelAppletClass *klass)
{
	GObjectClass   *gobject_class = (GObjectClass *) klass;
	GtkWidgetClass *widget_class = (GtkWidgetClass *) klass;
	GtkBindingSet *binding_set;

	gobject_class->get_property = gde2_panel_applet_get_property;
	gobject_class->set_property = gde2_panel_applet_set_property;
	gobject_class->constructed = gde2_panel_applet_constructed;
	klass->move_focus_out_of_applet = gde2_panel_applet_move_focus_out_of_applet;
#if GTK_CHECK_VERSION (3, 0, 0)
	klass->change_background = gde2_panel_applet_change_background;
#endif
	widget_class->button_press_event = gde2_panel_applet_button_press;
	widget_class->button_release_event = gde2_panel_applet_button_release;
#if GTK_CHECK_VERSION (3, 0, 0)
	widget_class->get_request_mode = gde2_panel_applet_get_request_mode;
#if !GTK_CHECK_VERSION (3, 18, 0)
	widget_class->get_preferred_width = gde2_panel_applet_get_preferred_width;
	widget_class->get_preferred_height = gde2_panel_applet_get_preferred_height;
#endif
	widget_class->draw = gde2_panel_applet_draw;
#else
	widget_class->size_request = gde2_panel_applet_size_request;
	widget_class->expose_event = gde2_panel_applet_expose;
#endif
	widget_class->size_allocate = gde2_panel_applet_size_allocate;
	widget_class->focus = gde2_panel_applet_focus;
	widget_class->realize = gde2_panel_applet_realize;
	widget_class->popup_menu = gde2_panel_applet_popup_menu;

	gobject_class->finalize = gde2_panel_applet_finalize;

	g_type_class_add_private (klass, sizeof (Gde2PanelAppletPrivate));
	...
	g_object_class_install_property (gobject_class,
					 ...
					 g_param_spec_uint ("orient",
							    ...
							    GDE2_PANEL_APPLET_ORIENT_FIRST,
							    GDE2_PANEL_APPLET_ORIENT_LAST,
	...
	g_object_class_install_property (gobject_class,
					 ...
					 g_param_spec_uint ("flags",
							    "Flags",
							    "Panel Applet flags",
							    GDE2_PANEL_APPLET_FLAGS_NONE,
							    GDE2_PANEL_APPLET_FLAGS_ALL,
							    GDE2_PANEL_APPLET_FLAGS_NONE,
							    G_PARAM_READWRITE));
	g_object_class_install_property (gobject_class,
					 PROP_SIZE_HINTS,
	...
	gde2_panel_applet_signals [CHANGE_ORIENT] =
                g_signal_new ("change_orient",
                              ...
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (Gde2PanelAppletClass, change_orient),
                              NULL,
			      NULL,
                              gde2_panel_applet_marshal_VOID__UINT,
                              G_TYPE_NONE,
			      1,
			      G_TYPE_UINT);
	...
	gde2_panel_applet_signals [CHANGE_SIZE] =
                g_signal_new ("change_size",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (Gde2PanelAppletClass, change_size),
                              NULL,
			      NULL,
                              gde2_panel_applet_marshal_VOID__INT,
                              G_TYPE_NONE,
			      1,
			      G_TYPE_INT);
	...
	gde2_panel_applet_signals [CHANGE_BACKGROUND] =
                g_signal_new ("change_background",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (Gde2PanelAppletClass, change_background),
                              NULL,
			      NULL,
                              gde2_panel_applet_marshal_VOID__ENUM_BOXED_OBJECT,
                              G_TYPE_NONE,
			      3,
			      PANEL_TYPE_GDE_2_PANEL_APPLET_BACKGROUND_TYPE,
#if GTK_CHECK_VERSION (3, 0, 0)
			      GDK_TYPE_RGBA,
	...
	gde2_panel_applet_signals [MOVE_FOCUS_OUT_OF_APPLET] =
                g_signal_new ("move_focus_out_of_applet",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                              G_STRUCT_OFFSET (Gde2PanelAppletClass, move_focus_out_of_applet),
                              NULL,
			      NULL,
                              gde2_panel_applet_marshal_VOID__ENUM,
                              G_TYPE_NONE,
			      1,
...
{
	Gde2PanelApplet* applet = g_object_new(PANEL_TYPE_APPLET, NULL);
...
method_call_cb (GDBusConnection       *connection,
		...
		gpointer               user_data)
{
	Gde2PanelApplet *applet = GDE2_PANEL_APPLET (user_data);
	...
	if (g_strcmp0 (method_name, "PopupMenu") == 0) {
		...
		gde2_panel_applet_menu_popup (applet, button, time);
		 ...
		 gpointer         user_data)
{
	...
	if (g_strcmp0 (property_name, "PrefsPath") == 0) {
		gde2_panel_applet_set_preferences_path (applet, g_variant_get_string (value, NULL));
	} else if (g_strcmp0 (property_name, "Orient") == 0) {
		gde2_panel_applet_set_orient (applet, g_variant_get_uint32 (value));
	} else if (g_strcmp0 (property_name, "Size") == 0) {
		gde2_panel_applet_set_size (applet, g_variant_get_uint32 (value));
	} else if (g_strcmp0 (property_name, "Background") == 0) {
		gde2_panel_applet_set_background_string (applet, g_variant_get_string (value, NULL));
	} else if (g_strcmp0 (property_name, "Flags") == 0) {
		gde2_panel_applet_set_flags (applet, g_variant_get_uint32 (value));
	} else if (g_strcmp0 (property_name, "SizeHints") == 0) {
		const int *size_hints;
		gsize      n_elements;
		...
		gde2_panel_applet_set_locked (applet, g_variant_get_boolean (value));
	} else if (g_strcmp0 (property_name, "LockedDown") == 0) {
		gde2_panel_applet_set_locked_down (applet, g_variant_get_boolean (value));
	}
...
static const gchar introspection_xml[] =
	"<node>"
	  "<interface name='org.gde2.panel.applet.Applet'>"
...
static void
gde2_panel_applet_register_object (Gde2PanelApplet *applet)
{
	GError     *error = NULL;
	static gint id = 0;
	...
	applet->priv->object_path = g_strdup_printf (GDE2_PANEL_APPLET_OBJECT_PATH, applet->priv->id, id++);
	applet->priv->object_id =
...
static void gde2_panel_applet_factory_main_finalized(gpointer data, GObject* object)
{
	gtk_main_quit();
...
/*
 * To do graphical embedding in the X window system, GDE2 Panel
 ...
 *
 * To solve this latter problem, GDE2 Panel sets up its own X error
 ...
 */
static void
_gde2_panel_applet_setup_x_error_handler (void)
{
...
/**
 * gde2_panel_applet_factory_main:
 ...
 */
int gde2_panel_applet_factory_main(const gchar* factory_id, gboolean out_process, GType applet_type, Gde2PanelAppletFactoryCallback callback, gpointer user_data)
{
	...
	if (out_process)
	{
		_gde2_panel_applet_setup_x_error_handler();
	}

	closure = g_cclosure_new(G_CALLBACK(callback), user_data, NULL);
	factory = gde2_panel_applet_factory_new(factory_id, applet_type, closure);
	g_closure_unref(closure);

	if (gde2_panel_applet_factory_register_service(factory))
	{
		if (out_process)
		{
			g_object_weak_ref(G_OBJECT(factory), gde2_panel_applet_factory_main_finalized, NULL);
			gtk_main();
		}
...
/**
 * gde2_panel_applet_set_background_widget:
...
void
gde2_panel_applet_set_background_widget (Gde2PanelApplet *applet,
				    GtkWidget   *widget)
{
}
				    ...
				    GtkWidget   *widget)
{
	...
	if (widget && gtk_widget_get_realized (widget)) {
		...
		cairo_pattern_t           *pattern;
		type = gde2_panel_applet_get_background (applet, &color, &pattern);
		_gde2_panel_applet_apply_css(widget,type);
				    ...
				    GtkWidget   *widget)
{
	...
	if (widget) {
		...
		GdkPixmap                 *pixmap;
		type = gde2_panel_applet_get_background (applet, &color, &pixmap);
		gde2_panel_applet_update_background_for_widget (widget, type,
							   &color, pixmap);
		if (type == PANEL_PIXMAP_BACKGROUND)
			g_object_unref (pixmap);
...
guint32
gde2_panel_applet_get_xid (Gde2PanelApplet *applet,
		      GdkScreen   *screen)
{
...
const gchar *
gde2_panel_applet_get_object_path (Gde2PanelApplet *applet)
{
